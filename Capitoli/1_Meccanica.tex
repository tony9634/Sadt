...
\[\dfrac{d}{dt}\dfrac{\partial L}{\partial\dot{\boldsymbol{\vartheta}}} - \dfrac{\partial L}{\partial\boldsymbol{\vartheta}} = \vec{0}\]

L'equazione si traduce in due equazioni, relative a \(\vartheta_{1}\) e \(\vartheta_{2}\):

\[ \begin{cases}
\dfrac{d}{dt}\left( \dfrac{\partial L}{\partial{\dot{\vartheta}}_{1}} \right) - \dfrac{\partial L}{\partial\vartheta_{1}} = 0 \\
\dfrac{d}{dt}\left( \dfrac{\partial L}{\partial{\dot{\vartheta}}_{2}} \right) - \dfrac{\partial L}{\partial\vartheta_{2}} = 0
\end{cases} \]
...
\begin{figure}[ht]
\includegraphics[width=6.25in,height=2.38333in]{media/1_Meccanica/doppio_pendolo.eps}
...
Il sistema può essere risolto mediante il \textbf{Metodo di Eulero esplicito}. Quest'ultimo si basa sull'approssimazione della derivata $dY / dt \approx \Delta Y / \Delta t$.

Si considera il generico passo temporale $t_k$. La soluzione al passo successivo $t_{k+1} = t_k + \Delta t$ è data da $Y_{k+1} = Y_k + \Delta Y$.

Per la prima equazione (ad esempio):
\[
