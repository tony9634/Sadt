...
Questa espressione rappresenta il teorema della \textbf{dinamica dei sistemi di particelle}, il quale afferma che la somma delle forze interne ed esterne agenti su un sistema di particelle è uguale alla derivata temporale della somma delle quantità di moto delle singole particelle. Poiché le forze interne si annullano reciprocamente in base al terzo principio della dinamica, \({\vec{F}}_{\text{tot}}\) coincide con la sommatoria delle forze esterne.
...
Conoscendo le forze esterne agenti, è possibile determinare il moto della particella. Integrando la Seconda Legge di Newton si ottiene l'impulso:

\[
\vec{F} = \dfrac{d\vec{p}}{dt}
\]

Per ottenere la posizione, si integra la velocità:

\[
\int_{t_{0}}^{t}{\vec{F}\, dt} = \vec{p}(t) - \vec{p}(t_{0}) = m\left[\vec{v}(t) - \vec{v}(t_{0})\right]
\]
...
\[\int_{t_{0}}^{t}{\left( \int_{t_{0}}^{t'}{\vec{F}\, dt''} \right)dt'} = m\lbrack\vec{r}(t) - \vec{r}(t_{0})\rbrack\]
...
\begin{align*}
\dfrac{d}{dt}\left(\dfrac{\partial L}{\partial\dot{q}_i}\right) - \dfrac{\partial L}{\partial q_i} &= 0, \quad \text{con } i=1,2,\dots,s
\end{align*}
...
Siccome le variazioni di spostamento e velocità sono molto minori delle rispettive \textbf{quantità}:
...
Per il Lemma Fondamentale del Calcolo delle Variazioni, la funzione integranda deve essere nulla:
...
Le due relazioni sono note come definizioni fondamentali della meccanica lagrangiana. La prima (\(p_{i}\)) è detta momento generalizzato coniugato, mentre la seconda forza generalizzata.
