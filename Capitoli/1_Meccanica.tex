...
La \textbf{meccanica classica} si fonda sulla descrizione dei fenomeni fisici secondo l'approccio introdotto da Newton nel XVII secolo. Essa interpreta il moto della natura in termini di forze $\vec{F}$ e accelerazioni $\vec{a}$. Il punto di vista \textbf{newtoniano} si basa sull'equazione fondamentale del moto:
...
In fisica teorica e meccanica analitica, quando si indica il vettore delle coordinate generalizzate $\mathbf{q} = (q_1, q_2, \ldots, q_s)$ nello spazio delle configurazioni, la convenzione più diffusa è usare il grassetto ($\mathbf{q}$) piuttosto che la freccia ($\vec{q}$).Questo perché la freccia ($\vec{q}$) è tradizionalmente riservata ai vettori nello spazio euclideo tridimensionale ($\mathbb{R}^3$), mentre $\mathbf{q}$ è un vettore in uno spazio $s$-dimensionale, dove $s$ non è necessariamente 3.

\input{Image/1_Meccanica/1_Traittoria.tex}
...
\[
T = T\left( q_{i},{\dot{q}}_{i} \right),\ i = 1,\ \ldots,\ s
\]
...
\[
x_{i} = f_{i}\left( q_{1},\ q_{2},\ \ldots,\ q_{s} \right),\quad i = 1,2,\ldots,N
\]
...
\[{\dot{x}}_{i} = \dfrac{d}{dt}f_{i}\left( q_{1},q_{2},\ldots,\ q_{s} \right),\quad i = 1,2,\ldots,N\]
...
È conveniente esprimere anche l'energia potenziale in funzione delle coordinate generalizzate. Si considera un sistema di \(N\) particelle. L'energia di interazione tra le particelle è indicata con \(U\left( q_{i} \right),\ i = 1,2,\ldots,s\) e viene denotata come energia potenziale. In meccanica classica questa quantità dipende solamente dalla posizione.
...
\[
L\left( q_{i},{\dot{q}}_{i} \right) = T\left( q_{i},{\dot{q}}_{i} \right) - U\left( q_{i} \right),\quad i = 1,\ldots,s
\]

dove \(s\) è il numero di gradi di libertà del sistema.

La lagrangiana \(L\left( q_{i}\left(t\right),{\dot{q}}_{i}\left(t\right) \right)\) è, dunque, una funzione delle coordinate generalizzate posizione e velocità, che a loro volta dipendono dal tempo:
...
\item Un elemento di \(TQ\) è una coppia \(\left( \vec{q},\dot{\vec{q}} \right)\), ovvero $\vec{q}$ è posizione (o configurazione) e velocità della particella.
\end{itemize}

Se la lagrangiana non dipende esplicitamente dal tempo essa agisce sul fibrato tangente a valori in \(\mathbb{R}\):
...
= \sum_{j} \frac{\partial x_j}{\partial q_i}\,\frac{\partial}{\partial x_j}
\]

Nell'ipotesi che la $i$-esima coordinata generalizzata $q_i$ coincida con la $i$-esima coordinata cartesiana $x_i$ e  che $x_i$ non dipende da nessuna delle altre coordinate generalizzate $q_j$ (per $j \neq i$), è possibile semplificare la relazione precedente:

\[
\dfrac{\partial}{\partial q_{i}} = \sum_{j} \dfrac{\partial x_{j}}{\partial q_{i}}\dfrac{\partial}{\partial x_{j}} = \sum_{j} \delta_{ij}\dfrac{\partial}{\partial x_{j}} = \dfrac{\partial}{\partial x_{i}}
\]

 Analogo procedimento può essere eseguito per passare da \({\dot{q}}_{i}\) a \({\dot{x}}_{i}\).
...
\displaystyle f_{i} = - \dfrac{\partial U}{\partial q_{i}}
\end{cases}
\]

Le due relazioni sono note come definizioni fondamentali della meccanica lagrangiana. La prima (\(p_{i}\)) è detta \textbf{momento generalizzato coniugato}, mentre la seconda forza generalizzata. Più in generale, si definisce la \textbf{forza generalizzata} $Q_i$ come:

\[
Q_i = \sum_j \vec{F}_j \cdot \frac{\partial \vec{r}_j}{\partial q_i}
\]

Cioè, è la proiezione di tutte le forze effettive sulle direzioni di variazione delle coordinate generalizzate.
...
questa relazione lega la derivata del momento generalizzato coniugato alla lagrangiana.
...
\[L = L\left( \vec{q},\dot{\vec{q}} \right),\quad \vec{q} = \vec{q}(t),\dot{\vec{q}} = \dot{\vec{q}}(t)\]
...
dove i prodotti tra vettori sono da intendersi come prodotti scalari. Dunque, ricorrendo alla simbologia sulle derivate temporali, si può scrivere:

\[
\dfrac{d}{dt}L\left( \vec{q},\dot{\vec{q}} \right) = \dfrac{\partial L}{\partial\vec{q}}\cdot\dot{\vec{q}} + \dfrac{\partial L}{\partial\dot{\vec{q}}}\cdot\vec{\ddot{q}}
\]

Si considera la quantità:

\[
\dfrac{d}{dt}\left( \dfrac{\partial L}{\partial\dot{\vec{q}}}\cdot\dot{\vec{q}} \right)
\]
...
Si considera la quantità \(C\), definita come:

\[
C = dfrac{\partial L}{\partial\dot{\vec{q}}}\dot{\vec{q}} - L
\]

La relazione si può scrivere come:

\[
\dfrac{d}{dt}\left( -C \right) = 0 \Leftrightarrow \dfrac{dC}{dt} = 0
\]

Integrando rispetto al tempo si ottiene:

\[
\dfrac{\partial L}{\partial\dot{\vec{q}}}\dot{\vec{q}} - L= \text{cost}
\]
...
\[
\dfrac{\partial L}{\partial\dot{\vec{q}}}\dot{\vec{q}} -L = \text{cost} \Leftrightarrow -T + U + 2T = cost
\]
...
In definitiva, si è dimostrato che la conservazione dell'energia totale (\(T+U\)) è una diretta conseguenza della simmetria temporale delle leggi fisiche. a conservazione dell'energia (il fatto che l'Hamiltoniana sia una costante del moto) si verifica solo se la Lagrangiana non dipende esplicitamente dal tempo.
...
La condizione di omogeneità spaziale impone che la lagrangiana non dipenda esplicitamente dalla posizione ($\vec{r}$), il che significa che la forza generalizzata è nulla
...
\[\delta\vec{r} = d\vec{\varphi} \times \vec{r},\quad \delta\vec{v} = d\vec{\varphi} \times \vec{v}\]
...
\displaystyle \dfrac{\partial L}{\partial{\vec{r}}_{i}} =  \displaystyle \dfrac{d{\vec{p}}_{i}}{dt}
\end{cases}
\]
...
\[\Delta h_{1} = l_{1} - l_{1}\cos\vartheta_{1} = l_{1}\left( 1 - cos\vartheta_{1} \right),\quad\Delta h_{2} = l_{1} + l_{2} - l_{1}\cos\vartheta_{1} - l_{2}\cos\vartheta_{2} = l_{1}\left( 1 - cos\vartheta_{1} \right) + l_{2}\left( 1 - cos\vartheta_{2} \right)\]
...
\subsection{Lemma 10: equazioni di Hamilton}\label{lemma-10-equazioni-di-hamilton}
...
Per semplificare la notazione si introduce la \textbf{parentesi di Poisson} \cite{arnold1992matematici}:
...
Il metodo di Eulero è un procedimento numerico per risolvere problemi con condizioni iniziali (IVP) che coinvolgono equazioni differenziali ordinarie.
...
\begin{figure}[ht]
\centering
\includegraphics[width=4.93333in,height=3.95833in,alt={P754\#yIS1}]{media/1_Meccanica/spettro_segnali.eps}\caption{Andamento delle soluzioni e relativa risposta spettrale}
\end{figure}
