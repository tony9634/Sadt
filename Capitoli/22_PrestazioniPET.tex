...
Detta \(d\) la dimensione del detettore, generalmente la risoluzione è variabile tre \(\dfrac{d}{2}\) al centro del \emph{Gantry} oppure \(d\) all'avvicinarsi del detettore. La risoluzione non è la stessa in tutto il \emph{Gantry}, ma dipende anche da fattori geometrici che riguardano la relazione geometrica tra detettori posti in coincidenza per determinate LOR.
...
La \emph{Sensitivity} è un parametro molto importante per identificare le \emph{performance} della PET, definita come il numero di eventi rilevati come coincidenza nell'unità di tempo per unità di attività nella sorgente. Essa è misurata in \(\dfrac{Cps}{MBq}\), ovvero conteggi per secondo rispetto ai Megabecquerel emessi dalla sorgente. Questo parametro dipende da:
...
\[R_{true} = \dfrac{C_{t} - C_{s}}{tempo\ di\ esposizione}\]
...
\[R_{t} = \dfrac{numero\ totale\ dieventi}{tempo\ di\ esposizione}\]
...
\[NECR\  = \dfrac{\left( R_{true} \right)^{2}}{R_{t}}\]
...
\[SUV = \dfrac{Act_{voi}}{\dfrac{Act_{administered}}{BW}}\lbrack = \rbrack\dfrac{\dfrac{kBq}{mL}}{\dfrac{MBq}{kg}}\ \]
