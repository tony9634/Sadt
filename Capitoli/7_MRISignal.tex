...
\begin{align*}
\text{emf}(t) \propto \int_{V'} \frac{\partial}{\partial t} \Bigg\{ 
& \left| M_{+}\left(\vec{r}, 0\right) \right| \exp\left( -\frac{t}{T_{2}\left(\vec{r}\right)} \right) \ \left[ \vec{B}_{\text{ric}}\left(\vec{r}\right) \cos\left( \omega_{0}\left(\vec{r}\right) t - \phi_{0}\left(\vec{r}\right) \right) \right. + \\
...
\begin{align*}
= \int_{V'}\! \Bigg\{ 
& - \frac{1}{T_{2}\!\left(\vec{r}'\right)} \left| M_{+}\!\left(\vec{r}, 0\right) \right| 
\exp\!\left( - \frac{t}{T_{2}\!\left(\vec{r}\right)} \right)
\Big[
...
\Big] + \\
& - \omega_{0}\!\left(\vec{r}\right) \left| M_{+}\!\left(\vec{r}, 0\right) \right| \exp\!\left( - \frac{t}{T_{2}\!\left(\vec{r}\right)} \right)\Big[
...
\[
\text{emf}\left(t\right) \propto \omega_0 \int_{V'} \left| M_+ \right| B_\perp \exp\left( - \frac{t}{T_2} \right) \sin\left( \omega_0 t - \phi_0 + \vartheta \right) dV'
= \omega_0 \left| M_+ \right| V_s B_\perp \exp\left( - \frac{t}{T_2} \right) \sin\left( \omega_0 t - \phi_0 + \vartheta \right)
\]
...
Per un campione omogeneo è semplice ottenere informazioni sulla fase. In particolare, avendo un segnale sinusoidale a frequenza \(\omega_0\) nota, il processo più semplice da utilizzare è la \emph{demodulazione coerente}, ovvero una demodulazione in cui si estraggono le informazioni di fase della sinusoide.
...
Mediante la demodulazione a due canali si riesce a ricostruire l'evoluzione sia della parte immaginaria che della parte immaginaria del segnale \(M_{+}\), ovvero le componenti longitudinali e trasversali del vettore di magnetizzazione \(\vec{M}\).
...
Si suppone di avere un campione di un materiale omogeneo, contenente un numero di Avogadro di spin. Immediatamente dopo l'impulso a radiofrequenza \({\vec{B}}_{1}\), la magnetizzazione, secondo l'equazione di Bloch, si porta dall'asse \({\hat{i}}_{x'}\) o \({\hat{i}}_{y'}\) al valore di equilibrio mediante una traiettoria ellittica.
...
Nel sistema di riferimento rotante, il campo \({\vec{B}}_{1}\) appare come un impulso costante sull'asse \({\hat{i}}_{x'}\) o \({\hat{i}}_{y'}\). Subito dopo l'esaurimento dell'impulso a radiofrequenze la magnetizzazione trasversale si riduce, in modulo, con costante di tempo \(T_{2}\).
...
\[s(t) \propto \omega_{0}\exp\left( - \dfrac{t}{T_{2}} \right)\exp\left\{ - j\left\lbrack \left( \omega_{0} - \omega \right)t + \phi_{0} - \vartheta_{B} \right\rbrack \right\}\int_{V}{{\vec{B}}_{\bot}\left( \vec{r} \right) \cdot {\vec{M}}_{\bot}\left( \vec{r} \right)dV}\]
...
\[B\left( \vec{r} \right) = B_{0} + \mathrm{\Delta}B\left( \vec{r} \right)\]
...
\[s(t) \propto \omega_{0}\int_{V}{\exp\left\lbrack - \dfrac{t}{T_{2}\left( \vec{r} \right)} \right\rbrack{\vec{B}}_{\bot}\left( \vec{r} \right) \cdot {\vec{M}}_{\bot}\left( \vec{r} \right)\sin\left\lbrack \omega_{0}\left( \vec{r} \right) \right\rbrack dV}\]

In linea di principio, per un materiale generico, il tempo di rilassamento traversale, così come la frequenza di precessione di Larmor, dipende dalla posizione \(\vec{r}\) del volumetto elementare nel corpo, anche a causa delle disomogeneità del campo \(\mathrm{\Delta}\vec{B}\).
...
\[s(t) \propto \omega_{0}\int_{V}{\exp\left\lbrack - \dfrac{t}{T_{2}\left( \vec{r} \right)} \right\rbrack{\vec{B}}_{\bot}\left( \vec{r} \right) \cdot {\vec{M}}_{\bot}\left( \vec{r},0 \right)\exp\left\{ - j\left\lbrack \left( \omega - \omega_{0} \right)t + \phi_{o}\left( \vec{r} \right) \right\rbrack \right\} dV}\]

Il segnale \(s(t)\) è proporzionale a un'oscillazione complessa \(\exp(j\mathrm{\Delta}\omega t)\). Si suppone che la fase iniziale dell'oscillazione sia nulla, ovvero \(\phi_{o}\left( \vec{r} \right) = 0\). Si considera, inoltre, un campione di materiale omogeneo come un bicchiere d'acqua. In questo caso, il tempo di rilassamento trasversale \(T_{2}\) non dipende dalla posizione. Si ritiene, infine, che l'antenna sia ideale, ovvero che il campo irradiato non dipende dalla posizione \(\vec{r}\). Il segnale registrato, sotto queste ipotesi, è proporzionala a:

\[s(t) \propto \omega_{0}\exp\left( - \dfrac{t}{T_{2}} \right)B_{\bot}\int_{V}{M_{\bot}\left( \vec{r},0 \right)\exp(j\mathrm{\Delta}\omega t)dV}\]
...
Il tempo di rilassamento \(T_{2}^{*}\) è strettamente legato alle disomogeneità di campo, infatti, per un \(\mathrm{\Delta}B = 1\ ppm\), il tempo \(T_{2}\) di un tessuto passa da \(100\ ms\) a \(5\ ms\) o minore. Inoltre, le disomogeneità di campo possono ridurre fortemente il segnale ottenuto. Per evitare ciò sono state proposte delle sequenze di impulsi per attenuare gli effetti della disomogeneità di campo. La prima proposta è la sequenza spin-echo, composta da due impulsi: il primo su un asse trasversale come \({\hat{i}}_{x'}\) che fa ruotare la magnetizzazione sul piano trasverso, allineandolo all'asse lungo cui è diretto l'impulso. Se, ad esempio, l'asse di rotazione è \(x'\), l'impulso è indicato con \((\pi/2)_{x'}\), in quanto ruota il vettore di magnetizzazione di un angolo pari a \(\pi/2\), così da precedere intorno all'asse \(x'\).

Il secondo impulso può essere diretto sia lungo \({\hat{i}}_{x'}\) sia lungo \({\hat{i}}_{y'}\) ed è di tipo \(\pi\), ovvero ruota la magnetizzazione nel piano trasverso di un angolo pari a \(\pi\)
...
%\input{Data/7_MRISignal_SpinEcho}
...
Infine, si definisce il vettore delle incognite, spesso indicato con il simbolo \(\vec{\vartheta}\), con due sole componenti:

\[\vec{\vartheta} = \left( \begin{array}{r}
m \\
q
\end{array} \right)\]
...
Si osservi che \(\vec{\vec{X}}\) è una matrice \(2 \times n\), quindi, non può essere invertita per ottenere la soluzione. In generale, la matrice dei coefficienti nel metodo dei minimi quadrati è di \(m \times n\), dove \(m\) è il numero delle incognite.
...
I vari elementi del vettore delle misure \(\vec{y}\) sono affetti da rumore approssimabile come variabili aleatori \(\varepsilon_{i}\). Sia:

\[\vec{\varepsilon} = \left( \begin{array}{r}
\varepsilon_{1} \\
\varepsilon_{2} \\
\ldots \\
\varepsilon_{n}
...
Si vuole trovare il vettore \(\vec{\vartheta}\) tale da minimizzare l'errore quadratico medio, ovvero:

\[\hat{\vec{\vartheta}} = {\arg{\min_{\vec{\vartheta}}\left\| \vec{\varepsilon} \right\|}}^{2} = \arg{\min_{\vec{\vartheta}}\left\| \vec{y} - \vec{\vec{X}}\vec{\vartheta} \right\|^{2}}\]

I parametri \(\vec{\vartheta}\) devono rendere minima la distanza tra le misure \(\vec{y}\) e le previsioni del modello \(\vec{\vec{X}}\vec{\vartheta}\).

La quantità \(\left\| \vec{y} - \vec{\vec{X}}\vec{\vartheta} \right\|^{2}\) può essere scritta in forma matriciale come:

\[\left\| \vec{y} - \vec{\vec{X}}\vec{\vartheta} \right\|^{2} = \left( \vec{y} - \vec{\vec{X}}\vec{\vartheta} \right)^{T}\left( \vec{y} - \vec{\vec{X}}\vec{\vartheta} \right)\]

Si indica con \(S\left( \vec{\vartheta} \right) = \left\| \vec{y} - \vec{\vec{X}}\vec{\vartheta} \right\|^{2}\), l'ultima relazione si scrive come:

\[S\left( \vec{\vartheta} \right) = \left( \vec{y} - \vec{\vec{X}}\vec{\vartheta} \right)^{T}\left( \vec{y} - \vec{\vec{X}}\vec{\vartheta} \right)\]

Svolgendo il trasposto e i prodotti si ha:

\[S\left( \vec{\vartheta} \right) = \left( \vec{y} - \vec{\vec{X}}\vec{\vartheta} \right)^{T}\left( \vec{y} - \vec{\vec{X}}\vec{\vartheta} \right) = \left( {\vec{y}}^{T} - {\vec{\vartheta}}^{T}{\vec{\vec{X}}}^{T} \right)\left( \vec{y} - \vec{\vec{X}}\vec{\vartheta} \right) = {\vec{y}}^{T}\vec{y} - {\vec{y}}^{T}\vec{\vec{X}}\vec{\vartheta} - {\vec{\vartheta}}^{T}{\vec{\vec{X}}}^{T}\vec{y} + {\vec{\vartheta}}^{T}{\vec{\vec{X}}}^{T}\vec{\vec{X}}\vec{\vartheta}\]

Le quantità \({\vec{y}}^{T}\vec{\vec{X}}\vec{\vartheta}\) e \({\vec{\vartheta}}^{T}{\vec{\vec{X}}}^{T}\vec{y}\) sono degli scalari, quindi, possono essere sommati tra loro:

\[S\left( \vec{\vartheta} \right) = {\vec{y}}^{T}\vec{y} + {\vec{\vec{X}}}^{T}\vec{\vec{X}}{\vec{\vartheta}}^{2} - 2{\vec{y}}^{T}\vec{\vec{X}}\vec{\vartheta}\]

Minimizzare il valore quadratico medio equivale a uguagliare a zero la derivata della quantità \(S\left( \vec{\vartheta} \right)\):

\[\dfrac{\partial S\left( \vec{\vartheta} \right)}{\partial\vec{\vartheta}} = 0 \Leftrightarrow 2{\vec{\vec{X}}}^{T}\vec{\vec{X}}\vec{\vartheta} - 2{\vec{y}}^{T}\vec{\vec{X}} = 0 \Leftrightarrow {\vec{\vec{X}}}^{T}\vec{\vec{X}}\vec{\vartheta} = {\vec{y}}^{T}\vec{\vec{X}}\]

Risulta che:

\[{\vec{y}}^{T}\vec{\vec{X}} = {\vec{\vec{X}}}^{T}\vec{y}\]

Per cui si ottiene:

\[{\vec{\vec{X}}}^{T}\vec{\vec{X}}\vec{\vartheta} = {\vec{\vec{X}}}^{T}\vec{y}\]

Moltiplicando a destra e a sinistra per \(\left( {\vec{\vec{X}}}^{T}\vec{\vec{X}} \right)^{- 1}\), si ottiene l'equazione per la soluzione \emph{ordinary least squares}:

\[\hat{\vec{\vartheta}} = \left( {\vec{\vec{X}}}^{T}\vec{\vec{X}} \right)^{- 1}{\vec{\vec{X}}}^{T}\vec{y}\]

Si suppone che il rumore sia a media nulla, ovvero:

\[E\left\lbrack \vec{\varepsilon} \right\rbrack = \vec{0}\]

La matrice della covarianza è data da:

\[E\left\lbrack \vec{\varepsilon}{\vec{\varepsilon}}^{T} \right\rbrack = \sigma^{2}\vec{\vec{I}}\]
...
\[E\left\lbrack \hat{\vec{\vartheta}} \right\rbrack = E\left\lbrack \left( {\vec{\vec{X}}}^{T}\vec{\vec{X}} \right)^{- 1}{\vec{\vec{X}}}^{T}\vec{y} \right\rbrack\]

Ma \(\vec{y} = \vec{\vec{X}}{\vec{\vartheta}}^{*} + \vec{\varepsilon}\); inoltre la matrice di design non contiene variabili aleatorie, quindi, può essere portato fuori dall'operazione di media statistica:

\[E\left\lbrack \hat{\vec{\vartheta}} \right\rbrack = E\left\lbrack \left( {\vec{\vec{X}}}^{T}\vec{\vec{X}} \right)^{- 1}{\vec{\vec{X}}}^{T}\vec{y} \right\rbrack = \left( {\vec{\vec{X}}}^{T}\vec{\vec{X}} \right)^{- 1}{\vec{\vec{X}}}^{T}E\left\lbrack \vec{y} \right\rbrack = \left( {\vec{\vec{X}}}^{T}\vec{\vec{X}} \right)^{- 1}{\vec{\vec{X}}}^{T}E\left\lbrack \vec{\vec{X}}{\vec{\vartheta}}^{*} + \vec{\varepsilon} \right\rbrack =\]

Per la linearità dell'operatore valor medio si ha:

\[= \left( {\vec{\vec{X}}}^{T}\vec{\vec{X}} \right)^{- 1}{\vec{\vec{X}}}^{T}\left( \vec{\vec{X}}E\left\lbrack {\vec{\vartheta}}^{*} \right\rbrack + E\left\lbrack \vec{\varepsilon} \right\rbrack \right)\]

Per ipotesi il rumore è a media nulla, per cui:

\[E\left\lbrack \hat{\vec{\vartheta}} \right\rbrack = \left( {\vec{\vec{X}}}^{T}\vec{\vec{X}} \right)^{- 1}{\vec{\vec{X}}}^{T}\vec{\vec{X}}E\left\lbrack {\vec{\vartheta}}^{*} \right\rbrack\]

Per definizione di matrice inversa risulta:

\[E\left\lbrack \hat{\vec{\vartheta}} \right\rbrack = E\left\lbrack {\vec{\vartheta}}^{*} \right\rbrack\]
...
\[E\left\lbrack \left( \hat{\vec{\vartheta}} - {\vec{\vartheta}}^{*} \right)\left( \hat{\vec{\vartheta}} - {\vec{\vartheta}}^{*} \right)^{T} \right\rbrack = E\left\lbrack \left( \left( {\vec{\vec{X}}}^{T}\vec{\vec{X}} \right)^{- 1}{\vec{\vec{X}}}^{T}\left( \vec{\vec{X}}{\vec{\vartheta}}^{*} + \vec{\varepsilon} \right) - {\vec{\vartheta}}^{*} \right)\left( \left( {\vec{\vec{X}}}^{T}\vec{\vec{X}} \right)^{- 1}{\vec{\vec{X}}}^{T}\left( \vec{\vec{X}}{\vec{\vartheta}}^{*} + \vec{\varepsilon} \right) - {\vec{\vartheta}}^{*} \right)^{T} \right\rbrack =\]

Svolgendo i prodotti si ha:

\[= E\left\lbrack \left( \left( {\vec{\vec{X}}}^{T}\vec{\vec{X}} \right)^{- 1}{\vec{\vec{X}}}^{T}\vec{\vec{X}}{\vec{\vartheta}}^{*} + \left( {\vec{\vec{X}}}^{T}\vec{\vec{X}} \right)^{- 1}{\vec{\vec{X}}}^{T}\vec{\varepsilon} - {\vec{\vartheta}}^{*} \right)\left( \left( {\vec{\vec{X}}}^{T}\vec{\vec{X}} \right)^{- 1}{\vec{\vec{X}}}^{T}\vec{\vec{X}}{\vec{\vartheta}}^{*} + \left( {\vec{\vec{X}}}^{T}\vec{\vec{X}} \right)^{- 1}{\vec{\vec{X}}}^{T}\vec{\varepsilon} - {\vec{\vartheta}}^{*} \right)^{T} \right\rbrack =\]

Per la proprietà della matrice inversa, si ha:

\[= E\left\lbrack \left( {\vec{\vartheta}}^{*} + \left( {\vec{\vec{X}}}^{T}\vec{\vec{X}} \right)^{- 1}{\vec{\vec{X}}}^{T}\vec{\varepsilon} - {\vec{\vartheta}}^{*} \right)\left( {\vec{\vartheta}}^{*} + \left( {\vec{\vec{X}}}^{T}\vec{\vec{X}} \right)^{- 1}{\vec{\vec{X}}}^{T}\vec{\varepsilon} - {\vec{\vartheta}}^{*} \right)^{T} \right\rbrack\]

Semplificando e svolgendo i prodotti si ha:

\[= E\left\lbrack \left( \left( {\vec{\vec{X}}}^{T}\vec{\vec{X}} \right)^{- 1}{\vec{\vec{X}}}^{T}\vec{\varepsilon} \right)\left( \left( {\vec{\vec{X}}}^{T}\vec{\vec{X}} \right)^{- 1}{\vec{\vec{X}}}^{T}\vec{\varepsilon} \right)^{T} \right\rbrack = E\left\lbrack \left( \left( {\vec{\vec{X}}}^{T}\vec{\vec{X}} \right)^{- 1}{\vec{\vec{X}}}^{T}\vec{\varepsilon} \right)\left( {\vec{\varepsilon}}^{T}\vec{\vec{X}}\left( {\vec{\vec{X}}}^{T}\vec{\vec{X}} \right)^{- T} \right) \right\rbrack = \left( {\vec{\vec{X}}}^{T}\vec{\vec{X}} \right)^{- 1}{\vec{\vec{X}}}^{T}E\left\lbrack \vec{\varepsilon}{\vec{\varepsilon}}^{T} \right\rbrack\vec{\vec{X}}\left( {\vec{\vec{X}}}^{T}\vec{\vec{X}} \right)^{- T}\]

Ma, per ipotesi \(E\left\lbrack \vec{\varepsilon}{\vec{\varepsilon}}^{T} \right\rbrack\vec{\vec{X}} = \sigma^{2}\vec{\vec{I}}\), per cui:

\[E\left\lbrack \left( \hat{\vec{\vartheta}} - {\vec{\vartheta}}^{*} \right)\left( \hat{\vec{\vartheta}} - {\vec{\vartheta}}^{*} \right)^{T} \right\rbrack = \left( {\vec{\vec{X}}}^{T}\vec{\vec{X}} \right)^{- 1}{\vec{\vec{X}}}^{T}\sigma^{2}\vec{\vec{I}}\vec{\vec{X}}\left( {\vec{\vec{X}}}^{T}\vec{\vec{X}} \right)^{- T} = \sigma^{2}\left( {\vec{\vec{X}}}^{T}\vec{\vec{X}} \right)^{- 1}{\vec{\vec{X}}}^{T}\vec{\vec{X}}\left( {\vec{\vec{X}}}^{T}\vec{\vec{X}} \right)^{- T}\]

Dove \({\vec{\vec{X}}}^{T}\vec{\vec{X}}\) è una matrice simmetrica, per cui l'inversa è anch'essa simmetrica, ovvero:

\[\left\lbrack \left( {\vec{\vec{X}}}^{T}\vec{\vec{X}} \right)^{- 1} \right\rbrack^{T} = \left\lbrack \left( {\vec{\vec{X}}}^{T}\vec{\vec{X}} \right)^{T} \right\rbrack^{- 1} = \left( {\vec{\vec{X}}}^{T}\vec{\vec{X}} \right)^{- 1}\]

Per cui:

\[\left( {\vec{\vec{X}}}^{T}\vec{\vec{X}} \right)^{- 1}{\vec{\vec{X}}}^{T}\vec{\vec{X}} = \vec{\vec{I}}\]

Di conseguenza:

\[E\left\lbrack \left( \hat{\vec{\vartheta}} - {\vec{\vartheta}}^{*} \right)\left( \hat{\vec{\vartheta}} - {\vec{\vartheta}}^{*} \right)^{T} \right\rbrack = \sigma^{2}\left( {\vec{\vec{X}}}^{T}\vec{\vec{X}} \right)^{- 1}\]
...
\begin{figure}[ht]
\centering
\includegraphics[width=3.95833in,height=4.19583in]{media/7_MRISignal/image213.pdf}\caption{Inversione della magnetizzazione dopo l'impulso a \(\pi\)}
\end{figure}
...
\begin{figure}[ht]
\centering
\includegraphics[width=5.65428in,height=3.825in]{media/7_MRISignal/image214.pdf}\caption{Andamento della componente longitudinale nel tempo}
\end{figure}
...
Per ottenere la misura di \(T_{1}\) attraverso la sequenza \emph{inversion recovery}, si osservi che al tempo \(t = 0^{+}\), a fine applicazione dell'impulso a \(\pi\), si ha una magnetizzazione orientata lungo \(- {\hat{i}}_{z}\):
...
\begin{figure}[ht]
\centering
\includegraphics[width=4.41074in,height=3.03691in,alt={Immagine che contiene diagramma, linea, Diagramma Descrizione generata automaticamente}]{media/7_MRISignal/image215.pdf}\caption{Andamento della magnetizzazione trasversale nel tempo nell'inversion recovery}
\end{figure}
