...
\input{Image/4_Quantistica/4_CorpoNero.tex}
}
...
\centering
\resizebox{0.5\textwidth}{!}{%
\input{Image/4_Quantistica/4_CatastrofeUV.tex}
}
...
\centering
\resizebox{0.5\textwidth}{!}{
\input{Image/4_Quantistica/4_SpectrumAbs.tex}
}
\caption{Spettro di assorbimento e di emissione per l'atomo di idrogeno}
\label{fig:4_SpectrumAbs}
\end{figure}
...
\begin{figure}[h!]
    ...
    \resizebox{1\textwidth}{!}{
        \input{Image/4_Quantistica/4_FeymannDiagram.tex}
    }
...
Il \textbf{Principio di Indeterminazione di Heisenberg} è uno dei fondamenti della Meccanica Quantistica (MQ) e riflette la sua natura intrinsecamente probabilistica e ondulatoria. Esso stabilisce un limite fondamentale alla precisione con cui è possibile conoscere simultaneamente due proprietà fisiche complementari (o \textbf{osservabili non commutabili}) di una particella. Secondo questo principio, non è possibile conoscere con precisione arbitraria e simultaneamente la \textbf{posizione} e la \textbf{quantità di moto} (o impulso) di una particella. Indicando con $\Delta x$ l'incertezza (la deviazione standard) sulla posizione e con $\Delta p$ quella sulla quantità di moto, la relazione è espressa dalla disuguaglianza:
...
dove $\hslash$ (h-bar) è la \textbf{costante di Planck ridotta} ($\hslash = \frac{h}{2\pi}$).

Questa relazione implica che, se la posizione viene determinata con estrema precisione ($\Delta x \rightarrow 0$), l'incertezza sulla quantità di moto deve necessariamente aumentare ($\Delta p \rightarrow \infty$) per mantenere valida la disuguaglianza. Analogamente, maggiore è la precisione sulla misura della quantità di moto ($\Delta p \rightarrow 0$), meno precisa è la conoscenza della posizione ($\Delta x \rightarrow \infty$).

Il principio può essere esteso anche alla coppia \textbf{energia-tempo} (sebbene la natura di $\Delta t$ sia concettualmente diversa rispetto a $\Delta x$ e $\Delta p$):

\[
\Delta E \Delta t \geq \dfrac{\hslash}{2}
\]

In questo contesto, $\Delta E$ rappresenta l'incertezza sull'energia e $\Delta t$ rappresenta tipicamente l'\textbf{intervallo di tempo minimo} necessario affinché l'energia del sistema possa essere definita (ad esempio, la vita media di uno stato instabile). Questo significa che una misura precisa dell'energia ($\Delta E \rightarrow 0$) richiede un intervallo di tempo $\Delta t$ sufficientemente lungo (tendente a infinito), mentre una misura molto rapida ($\Delta t$ piccolo) comporta una maggiore incertezza sull'energia ($\Delta E$ grande).

Il Principio di Indeterminazione \textbf{non è una conseguenza dei limiti della nostra strumentazione}, ma è una \textbf{proprietà fondamentale} dello stato quantistico. L'indeterminazione nasce dal fatto che gli operatori matematici che rappresentano la posizione ($\hat{x}$) e la quantità di moto ($\hat{p}$) \textbf{non commutano} (cioè, $[\hat{x}, \hat{p}] \neq 0$). Inoltre, il principio di indeterminazione è coerente con:

\begin{itemize}
    \item \textbf{Natura Ondulatoria}: L'indeterminazione è coerente con il \textbf{dualismo onda-particella}. Una particella ben localizzata ($\Delta x$ piccolo) corrisponde a un pacchetto d'onda formato dalla sovrapposizione di molte lunghezze d'onda (cioè, un ampio intervallo di momenti $\Delta p$).
    \item \textbf{Effetto della Misura}: L'atto di misurare, interagendo con la particella (ad esempio con fotoni), inevitabilmente rivela solo una parte della conoscenza intrinseca al sistema quantistico, portando al \textbf{collasso della funzione d'onda} e manifestando l'indeterminazione. Tuttavia, l'indeterminazione esiste indipendentemente da questo atto.
\end{itemize}

Nel mondo macroscopico, l'effetto di questo principio è trascurabile, poiché il valore di $\hslash$ è estremamente piccolo (circa $1.054 \times 10^{-34} \, \text{J} \cdot \text{s}$), rendendo l'incertezza minima ben al di sotto della nostra capacità di rilevazione.
