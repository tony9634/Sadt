...
La prima equazione è la \textbf{legge di Gauss per il campo elettrico} e afferma che la divergenza del campo elettrico è proporzionale alla densità di carica $\rho$ presente nel volume di riferimento, rapportata alla costante dielettrica nel vuoto \(\varepsilon_{0}\).

La seconda equazione è la \textbf{legge di Gauss per il magnetismo}, secondo cui la divergenza del campo di induzione magnetica $\vec{B}$ è nulla. Questa legge implica che le linee di campo magnetico non hanno sorgenti o pozzi (non esistono monopoli magnetici) e che il campo $\vec{B}$ è solenoidale. Dal punto di vista fisico, questa legge implica che le linee di campo magnetico entrano ed escono dal volumetto \(d\Omega\) su cui si calcola la divergenza, che, di conseguenza, è nulla \figurename~\ref{fig:3_MagneteLinee}.

La terza equazione è la \textbf{legge di Faraday-Neumann-Lenz} e stabilisce che un campo di induzione magnetica variabile nel tempo ($\partial\vec{B}/\partial t$) induce un campo elettrico rotazionale ($\vec{\nabla} \times \vec{E}$). In altre parole, la variazione del flusso del campo magnetico attraverso un circuito elettrico genera una forza elettromotrice indotta e, di conseguenza, una corrente elettrica indotta

\begin{figure}[ht]
\centering
\input{Image/3_Elettromagnetismo/3_MagneteLinee}
\caption{Flusso campo magnetico attraverso superficie chiusa}\label{fig:3_MagneteLinee}
\end{figure}
 
La quarta equazione è il \textbf{Teorema di Ampère-Maxwell}, secondo cui il rotore del campo $\vec{B}$ è determinato dalla densità di corrente di conduzione ($\vec{J}$) più la densità di corrente di spostamento ($\varepsilon_{0}\partial\vec{E}/\partial t$). La corrente di spostamento fu introdotta da Maxwell per garantire la conservazione della carica. Secondo questo teorema, dal punto di vista globale (forma integrale), la circolazione del campo $\vec{B}$ lungo un \textbf{circuito chiuso} è proporzionale alla corrente elettrica concatenata ($\mu_{0}I_{cond}$) più la corrente di spostamento ($\mu_{0}I_{spost}$) che attraversa \textbf{qualsiasi superficie aperta} delimitata dal circuito stesso.
...
Dunque, si è ottenuta la \textbf{legge di conservazione della carica}:
...
dove $c$ è la velocità di propagazione. Confrontando, si deduce che la velocità dell'onda elettromagnetica è data da:
...
Si vuole risolvere le equazioni di Maxwell in prossimità delle sorgenti (densità di corrente $\vec{J}$ e densità di carica $\rho$). Questo approccio permette di ricostruire l'intero campo elettromagnetico generato dalle sorgenti stesse.

Le equazioni di Maxwell nel vuoto con sorgenti sono:
\[
\begin{cases}
 \vec{\nabla} \cdot \vec{E} = \dfrac{\rho}{\varepsilon_{0}} & \text{(Legge di Gauss per } \vec{E}) \\
\vec{\nabla} \cdot \vec{B} = 0 & \text{(Legge di Gauss per } \vec{B})\\
\vec{\nabla} \times \vec{E} = - \dfrac{\partial\vec{B}}{\partial t} & \text{(Legge di Faraday-Neumann-Lenz)}  \\
\vec{\nabla} \times \vec{B} = \mu_{0}\left( \vec{J} + \varepsilon_{0}\dfrac{\partial\vec{E}}{\partial t} \right) & \text{(Legge di Ampère-Maxwell)}
\end{cases}
\]

Dall'equazione di Gauss per il magnetismo, risulta che il campo induzione magnetica è solenoidale, quindi, è possibile definire un \textbf{potenziale vettore} \(\vec{A}\) tale che:

\[
\vec{\nabla} \times \vec{A} = \vec{B}
\]

Sostituendo questa definizione nella terza equazione (Faraday-Neumann-Lenz):
...
I potenziali vettore $\vec{A}$ e scalare $\phi$ non sono univocamente determinati. Per tale motivo, si adotta una condizione di Gauge per disaccoppiare le equazioni differenziali. La Gauge di Lorentz, data da:
...
La soluzione nel dominio della frequenza per il potenziale vettore generato da una distribuzione di corrente $\vec{J}(\vec{r}')$ in un volume $V$ è data dall'integrale di convoluzione con la funzione di Green:

\[
\vec{A}(\vec{r}) = \dfrac{\mu_{0}}{4\pi} \int_{V} \dfrac{\vec{J}(\vec{r}')}{\left| \vec{r} - {\vec{r}}' \right|}e^{- jk\left| \vec{r} - {\vec{r}}' \right|} dV'
\]

In questa espressione, $\vec{r}'$ è la posizione della sorgente e $\vec{r}$ è il punto di valutazione del campo.
...
È la \textbf{funzione di Green} (o risposta impulsiva) per l'operatore di Helmholtz nel vuoto, che descrive la propagazione da una sorgente puntiforme.
...
Le equazioni di Maxwell in forma globale (o integrale) sono fisicamente equivalenti a quelle in forma locale. Si preferisce ricorrere alla forma globale in presenza di \textbf{alta simmetria} del sistema fisico (per semplificare i calcoli) o in presenza di \textbf{superfici di discontinuità} (condizioni al contorno tra materiali diversi).

Le equazioni di Maxwell nel vuoto in forma globale sono:
\[
\begin{cases}
 \oiint_{S}{\vec{E} \cdot d\vec{S}\ } = \dfrac{Q}{\varepsilon_{0}} & \text{(Legge di Gauss per } \vec{E}) \\
 \oiint_{S}{\vec{B} \cdot d\vec{S}\ } = 0 & \text{(Legge di Gauss per } \vec{B})  \\
 \oint_{\partial \Sigma}{\vec{E} \cdot d\vec{s}} = - \dfrac{\partial}{\partial t}\int_{\Sigma}{\vec{B} \cdot d\vec{\Sigma}} & \text{(Legge di Faraday)}  \\
 \oint_{\partial \Sigma}{\vec{B} \cdot d\vec{s}} = \mu_{0}I + \mu_{0}\varepsilon_{0}\dfrac{\partial}{\partial t}\int_{\Sigma}{\vec{E} \cdot d\vec{\Sigma}} & \text{(Legge di Ampère-Maxwell)}
\end{cases}
\]
...
In presenza di un \textbf{mezzo materiale} si introducono i vettori di \textbf{induzione elettrica} \(\vec{D}\) e di \textbf{campo magnetico} \(\vec{H}\). Il vettore \(\vec{B}\) è detto vettore di \textbf{induzione magnetica}, mentre \(\vec{E}\) rappresenta il \textbf{campo elettrico}.
