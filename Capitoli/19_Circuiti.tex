...
\vfill
    ...
    \label{blx:Circuiti\therefsection}
\vfill
...
\[\Gamma(d) = \dfrac{\dot{Z}(d) - {\dot{Z}}_{C}}{\dot{Z}(d) + {\dot{Z}}_{C}}\]
...
\[\left\{ \begin{array}{r}
v_{C}(t) \simeq \dfrac{Q}{C}\left( 1 - e^{- \lambda t} \right),\ \ t \ll \dfrac{1}{\theta} \\
v_{C}(t) \simeq \dfrac{Q}{C}e^{- \theta t},\ \ t \gg \dfrac{1}{\lambda}
\end{array} \right.\ \]
...
\[A \gg \dfrac{R_{2}}{R_{1}}\ \]

Allora la funzione di trasferimento è data da:

\[V_{out} = - \dfrac{R_{2}}{R_{1}}V_{in}\]
...
\[R_{1} = R_{2}||R_{3} = \dfrac{R_{2}R_{3}}{R_{2} + R_{3}}\]
...
\[\left\{ \begin{array}{r}
\dfrac{dv_{C}}{dt} + \dfrac{1}{R_{1}C_{1}}v_{C} = \dfrac{E_{in}}{R_{1}C_{1}} \\
v_{C}(0) = 0
\end{array} \right.\ \]

L'integrale generale dell'equazione differenziale è dato da:

\[v_{C}(t) = ke^{- \dfrac{t}{\tau_{d}}} + E_{in}\]

Imponendo la condizione iniziale:

\[v_{C}(t) = E_{in}\left( 1 - e^{- \dfrac{t}{\tau_{d}}} \right)\]

Questa tensione rappresenta il forzamento del secondo stadio, governato, quindi, dalle equazioni:

\[\left\{ \begin{array}{r}
\dfrac{dv_{C}}{dt} + \dfrac{1}{R_{2}C_{2}}v_{C} = \dfrac{E_{in}}{R_{2}C_{2}}\left( 1 - e^{- \dfrac{t}{\tau_{d}}} \right) \\
v_{C}(\infty) = 0
\end{array} \right.\ \]

La soluzione del problema è nota:

\[v_{C}(t) = \dfrac{\tau_{i}}{\left( \tau_{d} - \tau_{i} \right)}E_{in}\left( e^{- \dfrac{t}{\tau_{d}}} - e^{- \dfrac{t}{\tau_{i}}} \right)\]
...
\[\lim_{\tau_{i} \rightarrow \tau_{d}}{v_{C}(t)} = \lim_{\tau_{i} \rightarrow \tau_{d}}{\dfrac{\tau_{i}}{\left( \tau_{d} - \tau_{i} \right)}E_{in}\left( e^{- \dfrac{t}{\tau_{d}}} - e^{- \dfrac{t}{\tau_{i}}} \right)} = E_{in}\lim_{\tau_{i} \rightarrow \tau_{d}}{\dfrac{e^{- \dfrac{t}{\tau_{d}}}\left( 1 - e^{- \dfrac{t}{\tau_{i}}}e^{\dfrac{t}{\tau_{d}}} \right)\tau_{i}}{\tau_{d}\left( 1 - \dfrac{\tau_{i}}{\tau_{d}} \right)}\dfrac{{(E}_{in}e^{- \dfrac{t}{\tau_{d}}})}{\tau_{d}}} = = \lim_{\tau_{i} \rightarrow \tau_{d}}{\tau_{i}\dfrac{\left( 1 - e^{- \dfrac{t}{\tau_{i}}}e^{\dfrac{t}{\tau_{d}}} \right)}{\left( 1 - \dfrac{\tau_{i}}{\tau_{d}} \right)}} = \dfrac{t}{\tau_{d}}E_{in}e^{- \dfrac{t}{\tau_{d}}}\]
...
\[H(s) = \dfrac{A_{0}}{1 + \omega_{c}\left( C_{1}\left( R_{1} + R_{2} \right) + \left( 1 - A_{0} \right)R_{1}C_{2} \right)s + \omega_{c}^{2}R_{1}R_{2}C_{1}C_{2}s^{2}}\]
...
\[H(s) = \dfrac{R_{1}\left( R_{pz}C_{1}s + 1 \right)}{\left( 1 + s\tau_{2} \right)\left( R_{pz} + R_{1}\left( R_{pz}C_{1}s + 1 \right) \right)}\]

Il numeratore introduce uno zero e, quindi, rappresenta il termine di differenziazione del segnale.

Se si pone \(R_{pz} = \dfrac{\tau_{2}}{C_{1}}\) la funzione di trasferimento si riduce a una semplice cascata di filtri RC passivi:

\[H(s) = \dfrac{R_{1}}{\left( \dfrac{\tau_{2}}{C_{1}} + R_{1} + \dfrac{\tau_{1}\tau_{2}}{C_{1}}s \right)}\]
...
\[V_{S}(t) = V_{R} + R_{2}i(t) = V_{R} + \dfrac{R_{2}}{R_{1} + R_{2}}\left( V_{out}(t) - V_{R} \right)\]

Se la tensione di ingresso è minore della tensione di soglia superiore o \emph{High Threshold,} l'uscita sarà alta.

\[V_{S}^{H}(t) = V_{R} + \dfrac{R_{2}}{R_{1} + R_{2}}\left( V_{H} - V_{R} \right)\]
...
\[V_{S}^{L}(t) = V_{R} + \dfrac{R_{2}}{R_{1} + R_{2}}\left( V_{L} - V_{R} \right)\]
...
\[S = \dfrac{R_{2}}{R_{1} + R_{2}}\left( V_{H} - V_{L} \right)\]
