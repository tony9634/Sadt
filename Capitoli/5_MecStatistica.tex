...
\hline \hline
\textbf{Energia Interna} & \(U\) (o \(E\)) & \( U = \langle H \rangle = -\left(\frac{\partial \ln Z}{\partial \beta}\right)_{V, N}\) & Media dell'energia totale su tutti i microstati. \\
\hline
\textbf{Pressione} & \(P\) & \( P = \frac{1}{\beta}\left(\frac{\partial \ln Z}{\partial V}\right)_{T, N}\) & Legata all'impulso trasferito dagli urti delle particelle contro le pareti. \\
\hline
\textbf{Temperatura} & \(T\) & \( T = \frac{1}{k_B \beta}\) & Misura dell'agitazione termica media delle particelle. \\
\hline
\textbf{Entropia} & \(S\) & \(S = k_B \ln W\) & Logaritmo del numero \(W\) di microstati compatibili (disordine molecolare). \\
\hline
\textbf{Capacità Termica} & \(C_V\) & \( C_V = \left(\frac{\partial U}{\partial T}\right)_{V}\) & Misura della variazione dell'energia interna (media) con la temperatura. \\
\hline
\end{tabular}
\end{table}
...
\[
\frac{\partial}{\partial U_{1}} \ln g_{1} = \frac{1}{g_{1}} \frac{\partial g_{1}}{\partial U_{1}}
\]

Si ottiene:

\[
\frac{\partial}{\partial U_{1}} \ln g_{1} = \frac{\partial}{\partial U_{1}} \ln g_{2}
\]
...
Il termine esponenziale al secondo membro, \(\exp\left( - \frac{\varepsilon}{k_{B}T} \right)\), indica la probabilità relativa, non normalizzata, di un singolo stato, è chiamato fattore di Boltzmann.
...
\[
P(\varepsilon_{1,2}) = \frac{1}{Z} \exp\left( - \frac{\varepsilon_{1,2}}{k_B T} \right)
\]
...
\[
M \simeq N \frac{\gamma^2 \hslash^2}{4 k_B T} B_0
\]
...
\[
M \simeq \ N\frac{\gamma^{2}\hslash^{2}}{4k_{B}T}\left(\mu_0 H\right)
\]

In particolare, la suscettività magnetica è data da:

\[
\chi_{m} \simeq \mathbf{\mu_{0}} N\frac{\gamma^{2}\hslash^{2}}{4k_{B}T}
\]
...
\[
\sum_{s = 0}^{\infty}{s\exp( - xs)},\ \ x = \frac{\hslash\omega}{k_{B}T}
\]
...
\[
n_{\text{modi}} = \frac{\Delta f}{\delta f} = \frac{L}{c} \Delta f
\]
...
\[
\left\langle E \right\rangle = \frac{\hslash \omega}{\exp\left( \frac{\hslash \omega}{k_B T} \right) - 1}
\]

Quindi:

\[
E = 2 \cdot \frac{\Delta f}{\delta f} \cdot \frac{\hslash \omega}{\exp\left( \frac{\hslash \omega}{k_B T} \right) - 1}
\]
...
\[
\frac{P\left( N_{a},U_{a} \right)}{P\left( N_{b},U_{b} \right)} = \exp\left( \frac{\mu N_{a} - U_{a}}{k_{B}T} \right) \exp\left( - \frac{\mu N_{b} - U_{b}}{k_{B}T} \right) = \frac{\exp\left( \frac{\mu N_{a} - U_{a}}{k_{B}T} \right)}{\exp\left( \frac{\mu N_{b} - U_{b}}{k_{B}T} \right)}
\]
...
I fermioni sono particelle elementari con spin semi-intero, ad esempio \(\pm \frac{1}{2}\). Secondo il \textbf{principio di esclusione di Pauli}, due fermioni non possono occupare simultaneamente lo stesso stato quantico.
...
\[Z = \sum_{N}{\sum_{U}{\exp\left( \frac{\mu N - U}{k_{B}T} \right)}} = \underbrace{\exp\left( \frac{\mu \cdot 0 - 0}{k_{B}T} \right)}_{\text{Stato N=0, U=0}} + \underbrace{\exp\left( \frac{\mu \cdot 1 - \varepsilon}{k_{B}T} \right)}_{\text{Stato N=1, U}=\varepsilon} = 1 + \exp\left( \frac{\mu - \varepsilon}{k_{B}T} \right)\]
...
\[
\left\langle N(\varepsilon) \right\rangle = \frac{1}{Z}\sum_{N}{\sum_{U}{ N(\varepsilon)P(N,U)}}
\]

Poiché \(N = 0\) o \(1\), la somma si riduce al solo contributo del livello occupato:

\[
\left\langle N(\varepsilon) \right\rangle = \frac{1}{Z} \exp\left( \frac{\mu - \varepsilon}{k_B T} \right)
= \frac{\exp\left( \frac{\mu - \varepsilon}{k_B T} \right)}{1 + \exp\left( \frac{\mu - \varepsilon}{k_B T} \right)}
\]
...
\[
\left\langle N(\varepsilon) \right\rangle = \frac{1}{1 + \exp\left( \frac{\varepsilon - \mu}{k_B T} \right)}
\]
...
\[
\left\langle N(\varepsilon) \right\rangle = \frac{1}{\exp\left( \frac{\varepsilon - \mu}{k_{B}T} \right) - 1}
\]
