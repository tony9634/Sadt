...
\[J = 4\pi emk_{B}^{2}\frac{T}{h^{3}}e^{- \frac{\varphi}{k_{B}T}}\ \]
...
\[J = 4\pi emk_{B}^{2}\frac{T}{h^{3}}e^{- \frac{E_{A} + E_{G}}{k_{B}T}}\]
...
\[Q = \int_{0}^{\infty}{i(t)dt} = i_{0}\int_{0}^{\infty}{e^{- \lambda t}dt} = \frac{i_{0}}{\lambda} \Leftrightarrow i_{0} = Q\lambda\]
...
\[{\dot{I}}_{C} = \frac{{\dot{Y}}_{C}}{G + {\dot{Y}}_{C}}\dot{I} = \frac{j\omega C}{\frac{1}{R} + j\omega C}\dot{I}\]

\[\frac{j\omega C}{\frac{1}{R} + j\omega C} = \frac{j\omega CR}{1 + j\omega CR}\]
...
\[{\dot{V}}_{C} = \frac{1}{j\omega C}\frac{j\omega CR}{1 + j\omega CR}\dot{I} = \frac{R}{1 + j\omega CR}\dot{I}\]
...
\[Ri(t) = v_{C} + RC\frac{dv_{C}}{dt}\]

Particolarizzando al caso in esame, è noto il forzamento di tipo esponenziale:

\[RQ\lambda e^{- \lambda t} = v_{C} + RC\frac{dv_{C}}{dt}\]

Si pone \(\theta = \frac{1}{RC}\) reciproco della costante di tempo del circuito RC di prelievo del segnale, l'equazione della rete può essere scritta come:

\[Q\lambda e^{- \lambda t} = \frac{1}{R}v_{C} + \frac{1}{\theta R}\frac{dv_{C}}{dt}\]

Si passa all'omogenea associata:

\[\frac{1}{R}v_{C} + \frac{1}{\theta R}\frac{dv_{C}}{dt} = 0\]
...
\[Q\lambda e^{- \lambda t} = \frac{1}{R}Fe^{- \lambda t} - \frac{\lambda}{\theta R}Fe^{- \lambda t}\]

\[Q\lambda = \frac{F}{R}\left( 1 - \frac{\lambda}{\theta} \right) \Leftrightarrow F = \frac{RQ\lambda}{\left( 1 - \frac{\lambda}{\theta} \right)} = \frac{RQ\lambda}{(\theta - \lambda)}\theta = \frac{RQ\lambda}{(\theta - \lambda)}\frac{1}{RC} = \frac{1}{(\theta - \lambda)}\frac{Q\lambda}{C}\]

La soluzione particolare è, dunque:

\[u(t) = \frac{1}{(\theta - \lambda)}\frac{Q\lambda}{C}\ e^{- \lambda t} = - \frac{1}{(\lambda - \theta)}\frac{Q\lambda}{C}e^{- \lambda t}\ \]

L'integrale generale dell'equazione differenziale è, quindi, del tipo:

\[v_{C}(t) = ke^{- \theta t} - \frac{1}{(\lambda - \theta)}\frac{Q\lambda}{C}e^{- \lambda t}\]
...
\[k = \frac{1}{(\lambda - \theta)}\frac{Q\lambda}{C}\]

La soluzione della rete elettrica è stata determinata

\[v_{C}(t) = \frac{1}{(\lambda - \theta)}\frac{Q\lambda}{C}\left( e^{- \theta t} - e^{- \lambda t} \right)\]
...
\[\frac{1}{\lambda} \ll \frac{1}{\theta}\]

La tensione sulla capacità può essere espressa come:

\[v_{C}(t) \simeq \frac{Q}{C}\left( e^{- \theta t} - e^{- \lambda t} \right)\]
...
\[v_{C}(t) \simeq \frac{Q}{C}\left( 1 - e^{- \lambda t} \right)\]
...
\[v_{C}(t) \simeq \frac{Q}{C}e^{- \theta t}\]
...
\[\frac{1}{\lambda} \gg \frac{1}{\theta}\]

La tensione può essere scritta come:

\[v_{C}(t) \simeq \frac{Q}{C}\frac{\lambda}{\theta}\left( e^{- \theta t} - e^{- \lambda t} \right)\]
...
\[v_{C}(t) \simeq \frac{Q}{C}\frac{\lambda}{\theta}\left( 1 - e^{- \lambda t} \right)\]
...
\[v_{C}(t) \simeq \frac{Q}{C}\frac{\lambda}{\theta}e^{- \theta t}\]
...
Per costanti RC piccole, l'ampiezza del picco dipende da \(\frac{Q}{C}\frac{\lambda}{\theta}\), ovvero è proporzionale alla carica che a sua volta dipende dell'energia del fotone \(\gamma\) incidente. Tuttavia, essendo \(\frac{1}{\lambda} \gg \frac{1}{\theta}\), allora \(1 \gg \frac{\lambda}{\theta}\). L'impulso di tensione possiede un'ampiezza molto minore rispetto ai circuiti \emph{Large RC}.
