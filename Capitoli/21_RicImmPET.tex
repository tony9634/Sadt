...
\[\frac{N\left( d_{2} \right)}{N_{0}} = e^{- \int_{a}^{d_{2}}{\mu(s)ds}}\]
...
\[N\left( d_{1},d_{2} \right) = \int_{d_{1}}^{d_{2}}{\lambda(s)ds}\ e^{- \int_{d_{1}}^{d_{2}}{\mu(s)ds}} = \frac{N\left( d_{2} \right)}{N_{0}}\int_{d_{1}}^{d_{2}}{\lambda(s)ds}\]
...
\[N\left( d_{1},d_{2} \right)\frac{N_{0}}{N\left( d_{2} \right)}\  = \int_{d_{1}}^{d_{2}}{\lambda(s)ds}\]
...
Si può dimostrare che questo metodo introduce una \emph{Point Spread Function} o PSF del tipo \(\frac{1}{r}\) che provoca uno sfocamento dell'immagine ricostruita:

\[g(x,y) = f\left( \mathbf{r} \right)*\frac{1}{\left| \mathbf{r} \right|}\]
...
\[g(x,y) = f\left( \mathbf{r} \right)*\frac{1}{\left| \mathbf{r} \right|}*h\left( \mathbf{r} \right)\]
...
\[P_{N}(\lambda) = \frac{\lambda\left( \mathbf{r} \right)^{N\left( \mathbf{r} \right)}e^{- \lambda\left( \mathbf{r} \right)}}{N\left( \mathbf{r} \right)!}\]
...
\[SNR = \frac{E\left\lbrack P_{N} \right\rbrack}{VAR\left\lbrack P_{N} \right\rbrack} = \sqrt{\lambda}\]
...
\[\mathrm{\Delta}C_{j} = \frac{a_{ij}\left( p_{i} - q_{i} \right)}{\sum_{j}^{}a_{ij}} = \frac{a_{ij}\varepsilon}{\sum_{j}^{}a_{ij}}\]
...
\[p\left( \Lambda \middle| Q \right) = \frac{p\left( Q \middle| \Lambda \right)p(\Lambda)}{p(Q)}\ \]
...
\[p\left( q_{i} \middle| r_{i} \right) = \frac{r_{i}^{q_{i}}e^{- r_{i}}}{q_{i}!}\]
...
\[p\left( Q \middle| \Lambda \right) = \prod_{i}^{}\frac{r_{i}^{q_{i}}e^{- r_{i}}}{q_{i}!}\]
...
\[p\left( Q \middle| \Lambda \right) = \prod_{i}^{}\frac{r_{i}^{q_{i}}e^{- r_{i}}}{q_{i}!} \Leftrightarrow \log{p\left( Q \middle| \Lambda \right)} = \sum_{i}^{}\left( q_{i}\log r_{i} - r_{i} \right)\]
...
\[\frac{\partial L}{\partial\lambda_{j}} = \frac{\partial}{\partial\lambda_{j}}\sum_{i}^{}\left( q_{i}\log{\sum_{j = 1}^{J}{c_{ij}\lambda_{j}}}\  - \sum_{j = 1}^{J}{c_{ij}\lambda_{j}} \right) = 0\]

\[\frac{\partial}{\partial\lambda_{j}}\sum_{i}^{}{(q_{i}}\log{\sum_{j = 1}^{J}{c_{ij}\lambda_{j}}}\  - \sum_{j = 1}^{J}{c_{ij}\lambda_{j}}) = \sum_{i}^{}\left( \frac{q_{i}}{\sum_{j = 1}^{J}{c_{ij}\lambda_{j}}}\sum_{j = 1}^{J}c_{ij} - \sum_{j = 1}^{J}c_{ij} \right) = = \sum_{i}^{}\left( \left( \frac{q_{i}}{\sum_{j = 1}^{J}{c_{ij}\lambda_{j}}} - 1 \right)\sum_{j = 1}^{J}c_{ij} \right)\]

In definitiva, per massimizzare la funzione di \emph{Likelihood}, bisogna porre

\[\sum_{i}^{}\left( \left( \frac{q_{i}}{\sum_{j = 1}^{J}{c_{ij}\lambda_{j}}} - 1 \right)\sum_{j = 1}^{J}{c_{ij}\lambda_{j}} \right) = 0\]
...
\[n_{ij} = c_{ij}\lambda_{j}^{old}\frac{q_{i}}{\sum_{i}^{}{c_{ij}\lambda_{j}^{old}}}\]
...
\[\frac{\partial}{\partial\lambda_{j}}{E\lbrack L_{x}}(X,\Lambda)\left| \ Q,\Lambda^{old}\  \right\rbrack = \frac{\partial}{\partial\lambda_{j}}\sum_{i}^{}{\sum_{j = 1}^{J}{n_{ij}\ \log\left( c_{ij}\lambda_{j} \right) - c_{ij}\lambda_{j})}} = 0\]
...
\[\frac{\partial}{\partial\lambda_{j}}\sum_{i}^{}{\sum_{j = 1}^{J}{n_{ij}\log\left( c_{ij}\lambda_{j} \right) - c_{ij}\lambda_{j})}} = \sum_{i}^{}\left( \frac{n_{ij}}{\lambda_{j}} - c_{ij} \right)\]

Ponendo uguale a zero si ottiene l'attività del j-esimo pixel che massimizza la \emph{Likelihood}:

\[\sum_{i}^{}{\left( \frac{n_{ij}}{\lambda_{j}} - c_{ij} \right) = 0}\]

La cui soluzione è:

\[\lambda_{j} = \frac{\sum_{i}^{}n_{ij}}{\sum_{i}^{}c_{ij}}\]
...
\[\lambda_{j} = \lambda_{j}^{old}\frac{1}{\sum_{m}^{}c_{mj}}\ \sum_{i}^{}{c_{ij}\frac{q_{i}}{\sum_{k}^{}{c_{ik}\lambda_{k}^{old}}}}\]

Dove \(\frac{1}{\sum_{m}^{}c_{mj}}\) è la \emph{Sensitivity} dello scanner per il j-esimo pixel mentre \(\sum_{k}^{}{c_{ik}\lambda_{k}^{old}}\) è la proiezione che sarebbe misurata se l'immagine precedente fosse la vera immagine. Entrambi i fattori rappresentano dei pesi con cui stimare la nuova immagine.
