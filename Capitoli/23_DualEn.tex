...
\section{Rappresentazione dei coefficienti di attenuazione}\label{rappresentazione-dei-coefficienti-di-attenuazione}
...
\[\left\{ \begin{array}{r}
f_{p}(E) = \ \dfrac{1}{E^{3}}\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \  \\
f_{KN}(E) = \ \dfrac{1 + E}{E^{2}}\left\lbrack \dfrac{2(1 + E)}{1 + 2E} - \dfrac{1}{E}\ln(1 + 2E) \right\rbrack + \dfrac{1}{2E}\ln(1 + 2E) - \dfrac{(1 + 3E)}{(1 + 2E)^{2}}
\end{array} \right.\ \]
...
\[\left\{ \begin{array}{r}
a_{p} = K_{1}\dfrac{\rho}{A}Z^{n},\ \ n \simeq 4 \\
a_{c} = K_{2}\dfrac{\rho}{A}Z\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ 
\end{array} \right.\ \]
...
\section{Dual Energy Contrast-Enhanced Digital Mammography (DE-CEDM)}\label{dual-energy-contrast-enhanced-digital-mammography-de-cedm}
...
\[\log{\Phi^{H}(i,\ j)} - w_{I}\log{\Phi^{L}(i,\ j) =}k - \mu_{b}^{H}(T - t) - \mu_{I}^{H}t - w_{I}\left( - \mu_{b}^{L}(T - t) - \mu_{I}^{L}t \right) = = k - \mu_{b}^{H}(T - t) - \mu_{I}^{H}t - \ \dfrac{\mu_{b}^{H}}{\mu_{b}^{L}}\left( - \mu_{b}^{L}(T - t) - \mu_{I}^{L}t \right) = = k - \mu_{b}^{H}(T - t) - \mu_{I}^{H}t + \mu_{b}^{H}(T - t) + \dfrac{\mu_{b}^{H}}{\mu_{b}^{L}}\mu_{I}^{L}t = k - \mu_{I}^{H}t + w_{I}\mu_{I}^{L}t\]
...
\section{Attenuazione lineare}\label{attenuazione-lineare}
...
\[M(E) = - \log\dfrac{I}{I_{0}} = \int_{S}^{}{\mu(x,y,E)dS}\]
...
\section[Esempi di applicazione di attenuazione in Dual Energy]{Esempi di applicazione di attenuazione in \emph{Dual Energy}}
\label{esempi-di-applicazione-di-attenuazione-in-dual-energy}

\subsection{Identificazione di un materiale}\label{identificazione-di-un-materiale}
...
\[\left( \dfrac{\mu}{\rho} \right)_{x}(E) = c_{\alpha}^{x}\left( a_{p}^{\alpha}f_{p}(E) + \ a_{c}^{\alpha}f_{KN}(E) \right) + c_{\beta}^{x}\left( a_{p}^{\beta}f_{p}(E) + \ a_{c}^{\beta}f_{KN}(E) \right) = = \left( c_{\alpha}^{x}a_{p}^{\alpha} + c_{\beta}^{x}a_{p}^{\beta} \right)f_{p}(E) + \left( c_{\alpha}^{x}a_{c}^{\alpha} + c_{\beta}^{x}a_{c}^{\beta} \right)f_{KN}(E)\]
...
\subsection{Decomposizione in immagini di base}\label{decomposizione-in-immagini-di-base}
...
\[\theta^{\xi} = \tan^{- 1}\left( \dfrac{A_{\beta}^{\xi}}{A_{\alpha}^{\xi}} \right)\]

Se il mezzo è omogeneo allora:

\[\theta^{\xi} = \tan^{- 1}{\left( \dfrac{A_{\beta}^{\xi}}{A_{\alpha}^{\xi}} \right) = \tan^{- 1}\left( \dfrac{c_{\beta}^{\xi}L_{\xi}}{c_{\alpha}^{\xi}L_{\xi}} \right) = \tan^{- 1}\left( \dfrac{c_{\beta}^{\xi}}{c_{\alpha}^{\xi}} \right)}\]
...
\subsection{Identificazione del materiale}\label{identificazione-del-materiale}
...
E, da quest'ultimo è possibile ricavare il materiale, secondo la relazione \(\theta^{\xi} = \tan^{- 1}\left( \dfrac{c_{\beta}^{x}}{c_{\alpha}^{x}} \right)\). Ciò vale, ovviamente, se il materiale è unico.

\subsection{Identificazione con più materiali}\label{identificazione-con-piuxf9-materiali}
