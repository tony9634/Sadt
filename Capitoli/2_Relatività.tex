...
Verso la fine del 1800, emerse una profonda discordanza tra le leggi della \textbf{meccanica classica}, basate sulle trasformazioni di Galileo, e quelle dell'\textbf{elettromagnetismo}, descritte dalle equazioni di Maxwell. Le equazioni di Maxwell prevedevano che la luce, e più in generale ogni radiazione elettromagnetica, si propagasse nel vuoto con una velocità costante $c$, pari a circa $3 \times 10^8$ m/s. Tuttavia, se si applicavano le trasformazioni di Galileo, questa velocità avrebbe dovuto apparire diversa per osservatori in moto relativo, in contraddizione con la sua natura di costante universale.

L'\textbf{esperimento di Michelson-Morley}, condotto nel 1887, mirava originariamente a misurare la velocità della Terra rispetto al presunto \textbf{etere luminifero}, il mezzo attraverso cui si riteneva si propagasse la luce. Il celebre \textbf{risultato nullo} dell'esperimento dimostrò l'impossibilità di rilevare tale moto. Questa evidenza sperimentale supportò l'ipotesi che la velocità della luce $c$ è la stessa in tutti i sistemi di riferimento inerziali, indipendentemente dal moto della sorgente o dell'osservatore. Tale risultato sperimentale non era conciliabile con il Principio di Relatività galileiana applicato alle equazioni di Maxwell, rendendo necessaria la revisione delle fondamenta della fisica classica.
...
Da questo esempio discende che, cambiando sistema di riferimento, gli eventi che in un sistema di riferimento $K'$ sono contemporanei, non lo sono nel sistema di riferimento $K$. Si conclude, quindi, che il tempo è un concetto relativistico legato al sistema di riferimento considerato.
...
Einstein postulò un legame di tipo lineare tra le coordinate, noto come \textbf{Trasformazioni di Lorentz Generalizzate}:
...
\[
\begin{cases}
a^{2} - c^{2}g^{2} = 1 \quad &(1)\\
ab - c^{2}ge = 0 \quad &(2)\\
c^{2}e^{2} - b^{2} = c^{2} \quad &(3)\\
\end{cases}
\]
...
\[a\ dx + b\ dt = 0 \Leftrightarrow \dfrac{dx}{dt} = - \dfrac{b}{a}\]
...
\[\dfrac{dx}{dt} = - \dfrac{b}{a} \Leftrightarrow v = - \dfrac{b}{a}\]
...
\[\dfrac{b}{e} = - v \Leftrightarrow b = - ve\]
...
a = \gamma \\
b = - v\gamma \\
\displaystyle g = - \dfrac{v}{c^{2}}\gamma \\
e = \gamma
\end{cases} \Leftrightarrow \begin{cases}
a = \gamma \\
b = - \beta\gamma c \\
\displaystyle g = - \dfrac{\beta}{c}\gamma \\
e = \gamma
\end{cases} 
\]
...
y' = y \\
z' = z \\
\displaystyle t' = - \dfrac{\beta}{c}\gamma x + \gamma t
\end{cases}
\]
...
dy' = dy \\
dz' = dz \\
\displaystyle dt' = - \dfrac{\beta}{c}\gamma\ dx + \gamma\ dt
\end{cases}
\]
...
\[v_{x'} = \dfrac{dx'}{dt'} = \displaystyle \dfrac{\gamma(dx - \beta c\ dt)}{\gamma\left( - \dfrac{\beta}{c}\ dx + dt \right)} = \dfrac{dx - \beta c\ dt}{- \dfrac{\beta}{c}\ dx + dt}\]

Al secondo membro si mette in evidenza \(dt\):

\[v_{x'} = \dfrac{\dfrac{dx}{dt} - \beta c}{- \dfrac{\beta}{c}\dfrac{dx}{dt} + 1}\]
...
\[v_{x'} = \dfrac{v_{x} - \beta c}{- \dfrac{\beta}{c}v_{x} + 1}\]

Lungo \(y'\), la velocità è data da:

\[v_{y'} = \dfrac{dy'}{dt'} = \dfrac{dy}{\gamma\left( - \dfrac{\beta}{c}\ dx + dt \right)}\]

Moltiplicando e dividendo per \(dt\) al secondo membro, si ha:

\[v_{y'} = \dfrac{dy'}{dt'} = \dfrac{\dfrac{dy}{dt}}{\gamma\left( - \dfrac{\beta}{c}\dfrac{dx}{dt} + 1 \right)}\]

Dove compare la componente di velocità lungo l'asse \(x\) con cui si muove un fenomeno nel sistema \(K\):

\[\dfrac{dy}{dt} = v_{y} \Rightarrow v_{y'} = \dfrac{dy'}{dt'} = \dfrac{v_{y}}{\gamma\left( - \dfrac{\beta}{c}v_{x} + 1 \right)}\]

Analogamante, per la componente lungo \(z'\) si ha:

\[v_{z'} = \dfrac{dz'}{dt'} = \dfrac{dz}{\gamma\left( - \dfrac{\beta}{c}\ dx + dt \right)} = \dfrac{v_{z}}{\gamma\left( - \dfrac{\beta}{c}v_{x} + 1 \right)}\]
...
\[v_{x'} = \left. \ \dfrac{v_{x} - \beta c}{- \dfrac{\beta}{c}v_{x} + 1} \right|_{v_{x} = c} = \dfrac{c - \beta c}{- \dfrac{\beta}{c}c + 1} = c\dfrac{1 - \beta}{1 - \beta} = c\]
...
\[\gamma = \dfrac{1}{\sqrt{1 - \left( \dfrac{v}{c} \right)^{2}}} \simeq 1,\ \ \dfrac{v_{x}}{c} \ll 1\]
...
\[\gamma = \left. \ \dfrac{1}{\sqrt{1 - \left( \dfrac{v}{c} \right)^{2}}} \right|_{v = c} \rightarrow \infty\]
...
dy' = dy \\
dz' = dz \\
\displaystyle dt' = \gamma\left( - \dfrac{\beta}{c}dx + dt \right)
\end{cases}
\]
...
\displaystyle dt' = \gamma\left( - \dfrac{\beta}{c}dx + dt \right)
\end{cases}  \Leftrightarrow \begin{cases}
\displaystyle \dfrac{1}{\gamma}dx' = dx - \beta c\ dt \\
\displaystyle \dfrac{1}{\gamma}dt' = - \dfrac{\beta}{c}\ dx + dt
\end{cases} \Leftrightarrow \begin{cases}
\displaystyle dx = \dfrac{1}{\gamma}dx' + \beta c\ dt \\
\displaystyle dt = \dfrac{1}{\gamma}dt' + \dfrac{\beta}{c}\ dx
\end{cases}
\]
...
\begin{cases}
dx = \gamma\left( dx' + \beta c\ dt' \right) \\
\displaystyle dt = \gamma\left( dt' + \dfrac{\beta}{c}\ dx' \right)
\end{cases}
\]
...
\[
dx = \gamma\left( dx' + \beta c\ dt' \right) = \gamma\left( dx' + \beta c\ \left(- \dfrac{\beta}{c}\ dx' \right) \right) = 
\]
...
\[
= \gamma dx' \left(\dfrac{1}{\gamma^2}\right) =
\]
...
\[dt = \gamma d\tau = \dfrac{1}{\sqrt{1 - \dfrac{v^{2}}{c^{2}}}}d\tau\]
...
\begin{pmatrix}
\displaystyle c\dfrac{dt}{d\tau}, \displaystyle \dfrac{dx}{d\tau}, \displaystyle \dfrac{dy}{d\tau}, \displaystyle \dfrac{dz}{d\tau}
\end{pmatrix}
\]
...
\displaystyle c\dfrac{dt}{dt}, \displaystyle \dfrac{dx}{dt},  \displaystyle \dfrac{dy}{dt}, \displaystyle \dfrac{dz}{dt}
...
\[{u}^{\alpha} = \gamma \begin{pmatrix}
\displaystyle \dfrac{E}{m_{0}c}, \displaystyle\dfrac{1}{m_{0}}\vec{p}
\end{pmatrix}
\]
...
\[ \begin{cases}
\displaystyle t' = \gamma\left( - \dfrac{\beta}{c}x + t \right) \\
x' = \gamma(x - \beta ct) \\
y' = y \\
z' = z
\end{cases} 
...
\[
\begin{pmatrix}
\displaystyle \dfrac{E'}{m_{0}c} \\
\displaystyle \dfrac{p_{x'}}{m_{0}} \\
\displaystyle \dfrac{p_{y'}}{m_{0}} \\
\displaystyle \dfrac{p_{z'}}{m_{0}}
\end{pmatrix} = \begin{pmatrix}
\gamma & - \beta\gamma & 0 & 0 \\
 - \beta\gamma & \gamma & 0 & 0 \\
0 & 0 & 1 & 0 \\
...
\[
\begin{cases}
\displaystyle \dfrac{E'}{c} = \gamma\dfrac{E}{c} - \beta\gamma p_{x} \\
\displaystyle p_{x'} = - \beta\gamma\dfrac{E}{c} + p_{x}
\end{cases}
\]
...
\[\begin{cases}
p_{x_{1}} + p_{x_{2}} = 0 \\
\displaystyle - \beta\gamma\dfrac{E_{1}}{c} - \beta\gamma\dfrac{E_{2}}{c} = - \beta\gamma\dfrac{E_{3}}{c}
\end{cases} 
\]
