...
\begin{itemize}
\item \textbf{Aumentare il campo magnetico} \(B_0\): tipicamente si utilizzano campi da \(1.5\,\text{T}\), valore regolato da normative internazionali;
...
\mu_{x} & \mu_{y} & \mu_{z} \\
0 & 0 & B_{0}
\end{matrix} \right| = B_{0}\left( \mu_{y}{\hat{i}}_{x} - \mu_{x}{\hat{i}}_{y} \right)
\]

L'equazione differenziale scomposta lungo gli assi, si scrive come:

\[
\dfrac{d\vec{\mu}}{dt} = \gamma B_{0}\left( \mu_{y}{\hat{i}}_{x} - \mu_{x}{\hat{i}}_{y} \right)
\]
...
\left\lbrack - A\sin\left( \omega_{0}t \right) + B\cos\left( \omega_{0}t \right) \right\rbrack_{t = 0} = \mu_{y}(0)
...
\[
\begin{cases}
\mu_{x}\left(t\right) = \mu_{x}(0)\cos\left( \omega_{0}t \right) + \mu_{y}(0)\sin\left( \omega_{0}t \right) \\
\mu_{y}\left(t\right) = - \mu_{x}(0)\sin\left( \omega_{0}t \right) + \mu_{y}(0)\cos\left( \omega_{0}t \right) \\
\mu_{z}\left(t\right) = \mu_{z}(0)
\end{cases}
\]
...
\[
{\textbf{R}}_{z}\left( \omega_{0}t \right) = \begin{pmatrix}
...
\[\vec{\mu}\left(t\right) = {\textbf{R}}_{z}\left( \omega_{0}t \right)\vec{\mu}(0)\]
...
\[
\mu_{\perp}\left(t\right) = \mu_{\perp}(0)\exp\left( - j\omega_{0}t \right)
\]
...
\[
\angle\mu_{\perp} = \varphi\left(t\right) + \angle\mu(0) = - j\omega_{0}t + \varphi_{0}
\]

Dove \(\varphi_0 = \angle \mu_{\perp}(0)\) è la fase iniziale. L'evoluzione del fasore \(\mu_{\perp}\) può essere scritto come:

\[
\mu_{\perp}\left(t\right) = \left| \mu_{\perp}(0) \right|\exp\left( - j\left( \omega_{0}t + \varphi_{0} \right) \right)
\]
...
\section{\texorpdfstring{Sistema di riferimento rotante con velocità angolare \(\boldsymbol{\omega}\)}{Sistema di riferimento rotante con velocità angolare omega}}\label{sistema-di-riferimento-rotante-con-velocita-angolare-omega}
...
\[
\vec{c}\left(t\right) = c_{x}\left(t\right){\hat{i}}_{x} + c_{y}\left(t\right){\hat{i}}_{y} + c_{z}\left(t\right){\hat{i}}_{z}
\]
...
Dato che i due sistemi possiedono l'asse \(z\) in comune, deve risultare \({\hat{i}}_{z} \equiv {\hat{i}}_{z'}\), per cui:
...
\[
\dfrac{d\vec{c}}{dt} = \dfrac{d}{dt}\left( c_{x}\left(t\right){\hat{i}}_{x} + c_{y}\left(t\right){\hat{i}}_{y} + c_{z}\left(t\right){\hat{i}}_{z} \right) = \dfrac{dc_{x}}{dt}{\hat{i}}_{x} + c_{x}\dfrac{d{\hat{i}}_{x}}{dt} + \dfrac{dc_{y}}{dt}{\hat{i}}_{y} + c_{y}\dfrac{d{\hat{i}}_{y}}{dt} + \dfrac{dc_{z}}{dt}{\hat{i}}_{z} + c_{z}\dfrac{d{\hat{i}}_{z}}{dt}
\]

Poiché il sistema di riferimento è fisso, le derivate dei suoi versori sono nulle:

\[
\dfrac{d{\hat{i}}_{x}}{dt} = 0,\ \ \dfrac{d{\hat{i}}_{y}}{dt} = 0,\ \ \dfrac{d{\hat{i}}_{z}}{dt} = 0
\]

Si ottiene:

\[
\dfrac{d\vec{c}}{dt} = \dfrac{dc_{x}}{dt}{\hat{i}}_{x} + \dfrac{dc_{y}}{dt}{\hat{i}}_{y} + \dfrac{dc_{z}}{dt}{\hat{i}}_{z}
\]

Nel sistema rotante, invece, la derivata è:

\[
\dfrac{d\vec{c}}{dt} = \dfrac{dc_{x'}}{dt}{\hat{i}}_{x'} + c_{x'}\dfrac{d{\hat{i}}_{x'}}{dt} + \dfrac{dc_{y'}}{dt}{\hat{i}}_{y'} + c_{y'}\dfrac{d{\hat{i}}_{y'}}{dt} + \dfrac{dc_{z'}}{dt}{\hat{i}}_{z'} + c_{z'}\dfrac{d{\hat{i}}_{z'}}{dt}
\]
...
\[
\dfrac{d{\hat{i}}_{x'}}{dt} = \vec{\Omega} \times {\hat{i}}_{x'},\ \ \dfrac{d{\hat{i}}_{y'}}{dt} = \vec{\Omega} \times {\hat{i}}_{y'},\ \ \dfrac{d{\hat{i}}_{z'}}{dt} = \vec{\Omega} \times {\hat{i}}_{z'}
\]
...
\begin{align}
\dfrac{d\vec{c}}{dt} &=
\dfrac{dc_{x'}}{dt}{\hat{i}}_{x'} + c_{x'}\vec{\Omega} \times {\hat{i}}_{x'}
 + \dfrac{dc_{y'}}{dt}{\hat{i}}_{y'} + c_{y'}\vec{\Omega} \times {\hat{i}}_{y'}  + \dfrac{dc_{z'}}{dt}{\hat{i}}_{z'} + c_{z'}\vec{\Omega} \times {\hat{i}}_{z'} \nonumber\\[6pt]
&= \dfrac{dc_{x'}}{dt}{\hat{i}}_{x'} +
\dfrac{dc_{y'}}{dt}{\hat{i}}_{y'} +
\dfrac{dc_{z'}}{dt}{\hat{i}}_{z'} +  \vec{\Omega} \times
\left(c_{x'}{\hat{i}}_{x'} + c_{y'}{\hat{i}}_{y'} + c_{z'}{\hat{i}}_{z'}\right)
\end{align}

dove \({\vec{c}}' = c_{x'}{\hat{i}}_{x'} + c_{y'}{\hat{i}}_{y'} + c_{z'}{\hat{i}}_{z'}\) è il vettore nel sistema di riferimento fisso. Si indica con:

\[
\left( \dfrac{d\vec{c}}{dt} \right)' = \dfrac{dc_{x'}}{dt}{\hat{i}}_{x'} + \dfrac{dc_{y'}}{dt}{\hat{i}}_{y'} + \dfrac{dc_{z'}}{dt}{\hat{i}}_{z'}
\]
...
\[
\vec{\Omega} = - \omega{\hat{i}}_{z}
\]

In queste condizioni, il campo efficace è:

\[
{\vec{B}_{\text{eff}}} = B_{0}{\hat{i}}_{z} - \dfrac{1}{\gamma}\omega{\hat{i}}_{z}
\]

Se la frequenza con cui ruota il sistema coincide con quella di Larmor, ovvero \(\omega = \omega_{0} = \gamma B_{0}\), allora:

\[
{\vec{B}_{\text{eff}}} = B_{0}{\hat{i}}_{z} - \dfrac{1}{\gamma}\omega_{0}{\hat{i}}_{z} = B_{0}{\hat{i}}_{z} - \dfrac{1}{\gamma}\gamma B_{0}{\hat{i}}_{z} = \vec{0}
\]
...
\[{\vec{B}}_{x}\left(t\right) = b_{x}\left(t\right)\cos\left( \omega_{0}t \right){\hat{i}}_{x}\]

Nell'espressione vi è la dipendenza addizionale dell'addizionale dell'ampiezza delle oscillazioni \(b_{x}\left(t\right)\), poiché il segnale emanato è, solitamente, un pacchetto di sinusoidi.
...
\[\left\{ \begin{matrix}
{\hat{i}}_{x'} = {\hat{i}}_{x}\cos\left( \omega_{0}t \right) - {\hat{i}}_{y}\sin\left( \omega_{0}t \right) \\
{\hat{i}}_{y'} = {\hat{i}}_{x}\sin\left( \omega_{0}t \right) + {\hat{i}}_{y}\cos\left( \omega_{0}t \right)
\end{matrix} \right.\ \]
...
\[{\vec{B}}_{x}\left(t\right) = b_{x}\left(t\right)\cos\left( \omega_{0}t \right){\hat{i}}_{x} = b_{x}\left(t\right)\cos\left( \omega_{0}t \right)\left\lbrack {\hat{i}}_{x'}\cos\left( \omega_{0}t \right) + {\hat{i}}_{y'}\sin\left( \omega_{0}t \right) \right\rbrack = b_{x}\left(t\right)\cos^{2}\left( \omega_{0}t \right){\hat{i}}_{x'} + b_{x}\left(t\right)\cos\left( \omega_{0}t \right)\sin\left( \omega_{0}t \right){\hat{i}}_{y'}\]

Per le formule di duplicazione e addizione, si ottiene:

\[{\vec{B}}_{x}\left(t\right) = b_{x}\left(t\right)\left\lbrack \dfrac{1}{2}\cos\left( 2\omega_{0}t \right) + \dfrac{1}{2} \right\rbrack{\hat{i}}_{x'} + \dfrac{1}{2}b_{x}\left(t\right)\sin\left( 2\omega_{0}t \right){\hat{i}}_{y'}\]
...
\[{\vec{B}}_{x}\left(t\right) = \dfrac{1}{2}b_{x}\left(t\right){\hat{i}}_{x'} + b_{x}\left(t\right)\left\lbrack \dfrac{1}{2}\cos\left( 2\omega_{0}t \right){\hat{i}}_{x'} + \dfrac{1}{2}\sin\left( 2\omega_{0}t \right){\hat{i}}_{y'} \right\rbrack\]
...
\[\left\langle {\vec{B}}_{x}\left(t\right) \right\rangle = \dfrac{1}{T}\int_{T}^{}{{\vec{B}}_{x}\left(t\right)dt} = \dfrac{1}{2T}\int_{T}^{}{\left| \left\{ b_{x}\left(t\right){\hat{i}}_{x'} + b_{x}\left(t\right)\left\lbrack \cos\left( 2\omega_{0}t \right){\hat{i}}_{x'} + \sin\left( 2\omega_{0}t \right){\hat{i}}_{y'} \right\rbrack \right\} \right|dt} = \dfrac{1}{2}\left\langle b_{x}\left(t\right) \right\rangle\]
...
\[\vec{B}\left(t\right) = B_{1}\left\lbrack \cos\left( \omega_{0}t \right){\hat{i}}_{x} - \sin\left( \omega_{0}t \right){\hat{i}}_{x} \right\rbrack\]
...
{\hat{i}}_{x} = \cos\left( \omega_{0}t \right){\hat{i}}_{x'} + \sin\left( \omega_{0}t \right){\hat{i}}_{y'} \\
{\hat{i}}_{y} = - \sin\left( \omega_{0}t \right){\hat{i}}_{x'} + \cos\left( \omega_{0}t \right){\hat{i}}_{y'}
\end{matrix} \right.\ \]

Sostituendo queste relazioni nell'espressione del campo magnetico circolare si ha:

\[\vec{B}\left(t\right) = B_{1}\left\lbrack \cos\left( \omega_{0}t \right){\hat{i}}_{x} - \sin\left( \omega_{0}t \right){\hat{i}}_{x} \right\rbrack = B_{1}\left\{ \cos\left( \omega_{0}t \right)\left\lbrack \cos\left( \omega_{0}t \right){\hat{i}}_{x'} + \sin\left( \omega_{0}t \right){\hat{i}}_{y'} \right\rbrack - \sin\left( \omega_{0}t \right)\left\lbrack - \sin\left( \omega_{0}t \right){\hat{i}}_{x'} + \cos\left( \omega_{0}t \right){\hat{i}}_{y'} \right\rbrack \right\} = B_{1}\left\{ \cos^{2}\left( \omega_{0}t \right){\hat{i}}_{x'} + \cos\left( \omega_{0}t \right)\sin\left( \omega_{0}t \right){\hat{i}}_{y'} + \sin^{2}\left( \omega_{0}t \right){\hat{i}}_{x'} - \sin\left( \omega_{0}t \right)\cos\left( \omega_{0}t \right){\hat{i}}_{y'} \right\} = B_{1}\left\lbrack \cos^{2}\left( \omega_{0}t \right){\hat{i}}_{x'} + \sin^{2}\left( \omega_{0}t \right){\hat{i}}_{x'} \right\rbrack = B_{1}{\hat{i}}_{x'}\]
...
\(B_{0}{\hat{i}}_{z}\) è il campo statico esterno o principale applicato.

Si applica, ora, un campo a radiofrequenza. Se il sistema di riferimento ruota con la stessa pulsazione \(\omega_{0}\) del campo magnetico applicato, al campo effettivo, \({\vec{B}_{\text{eff}}}\), va aggiunto un termine costante, diretto lungo \({\hat{i}}_{x'}\):

\[\left( \dfrac{d\vec{\mu}}{dt} \right)' = \gamma\vec{\mu} \times \left( B_{0}{\hat{i}}_{z} - \dfrac{1}{\gamma}\omega{\hat{i}}_{z} + B_{1}{\hat{i}}_{x'} \right)\]

I due sistemi di riferimento possiedono gli assi \(z\) paralleli, per cui \({\hat{i}}_{z} = {\hat{i}}_{z'}\).
...
\[\left( \dfrac{d\vec{\mu}}{dt} \right)' = \gamma B_{1}\vec{\mu} \times {\hat{i}}_{x'}\]

Si assiste, dunque, a una processione degli spin intorno all'asse \({\hat{i}}_{x}'\)
...
L'introduzione del sistema rotante consente di descrivere in modo semplice il comportamento degli spin, mediante un moto di precessione intorno all'asse \(x'\). Nel sistema di riferimento fisso del laboratorio, invece, il contributo del campo a radiofrequenza si somma a quello stato e omogeneo, diretto lungo \({\hat{i}}_{z}\). Si dimostra che la combinazione dei due campi produce un modo elicoidale, il cui raggio aumento all'avvicinarsi del piano \(xy\).
...
\[{\vec{B}_{\text{eff}}} = B_{0}{\hat{i}}_{z} - \dfrac{1}{\gamma}\omega{\hat{i}}_{z} + B_{1}{\hat{i}}_{x'}\]
...
\[{\vec{B}_{\text{eff}}} = \dfrac{1}{\gamma}\left\lbrack \left( \omega_{0} - \omega \right){\hat{i}}_{z} + \omega_{1}{\hat{i}}_{x'} \right\rbrack\]
...
\[B_{1} = \dfrac{\mathrm{\Delta}\vartheta}{\gamma\tau} = \dfrac{\dfrac{\pi}{2}}{42.6\ \dfrac{MHz}{T}1\ ms} \simeq 5.9\ \mu T\]
...
L'uso del sistema rotante intorno all'asse \({\hat{i}}_{z}\) permette di descrivere un campo a polarizzazione circolare in modo molto semplice. Inoltre, se la frequenza del campo a radiofrequenza e quella del sistema rotante sono uguali a quella di Larmor, è possibile scrivere la relazione:

\[\left( \dfrac{d\vec{\mu}}{dt} \right)' = \gamma B_{1}\vec{\mu} \times {\hat{i}}_{x'}\]

Dove \({\vec{B}}_{1} = B_{1}{\hat{i}}_{x'}\) è il campo magnetico polarizzato circolarmente visto nel sistema rotante. La rotazione intorno all'asse \({\hat{i}}_{z}\), nel sistema fisso, si scrive come:

\[\vec{\mu}\left(t\right) = {\textbf{R}}_{z}(\omega t)\vec{\mu}(0)\]

Dove \({\textbf{R}}_{z}\) è la matrice di rotazione intorno all'asse \({\hat{i}}_{z}\):

\[{\textbf{R}}_{z} = \begin{pmatrix}
\cos\left( \omega_{0}t \right) & - \sin\left( \omega_{0}t \right) & 0 \\
\sin\left( \omega_{0}t \right) & \cos\left( \omega_{0}t \right) & 0 \\
0 & 0 & 1
\end{pmatrix}\]

Invece di ruotare il sistema lungo l'asse \({\hat{i}}_{z}\), si suppone che la rotazione avvenga intorno all'asse \({\hat{i}}_{y}\). In questo caso, la soluzione si scrive come:

\[\vec{\mu}\left(t\right) = {\textbf{R}}_{y}(\omega t)\vec{\mu}(0)\]

Dove:

\[{\textbf{R}}_{y} = \begin{pmatrix}
\cos\left( \omega_{0}t \right) & 0 & - \sin\left( \omega_{0}t \right) \\
0 & 1 & 0 \\
\sin\left( \omega_{0}t \right) & 0 & \cos\left( \omega_{0}t \right)
\end{pmatrix}\]

Con rotazione del sistema attorno all'asse \({\hat{i}}_{x}\), la soluzione è:

\[\vec{\mu}\left(t\right) = {\textbf{R}}_{x}(\omega t)\vec{\mu}(0)\]
...
Nel caso generale, in cui l'ampiezza del campo a radiofrequenza cambia nel tempo, ovvero \(B_{1} = B_{1}\left(t\right)\), la fase all'istante fissato \(\tau\), è ottenuto come integrale temporale, infatti:
...
Si suppone di applicare un campo a radiofrequenza \(B_{1}\), diretto lungo \({\hat{i}}_{x'}\), tale da far ruotare gli spin di un angolo \(\vartheta\) rispetto all'asse \({\hat{i}}_{x'}\). Si applica, non appena viene interrotta la trasmissione del primo campo, un secondo impulso a radiofrequenza, diretto lungo l'asse \({\hat{i}}_{y'}\). La descrizione del moto di precessione, nel sistema di riferimento fisso, in questo scenario, è dato da:

\[\vec{\mu}\left(t\right) = {\textbf{R}}_{y}{\textbf{R}}_{x}(\omega t)\vec{\mu}(0)\]
...
Dato l'elevato numero di spin si introduce il vettore di magnetizzazione su unità di volume \(\vec{M}\).

Si posiziona un volumetto elementare contenente un numero di Avogadro \(N_{A}\) di protoni, in un campo magnetico diretto lungo \({\hat{i}}_{z}\). Gli spin nel volumetto si allineano rispetto all'asse del campo magnetico esterno.

\begin{figure}[ht]
\centering
\includegraphics[width=2.89286in,height=2.63294in,alt={P3086\#yIS1}]{media/6_IntroMRI/image66.pdf}\caption{Volumetto elementare di \(N_{A}\) di protoni in un campo magnetico diretto lungo \({\hat{i}}_{z}\)}
\end{figure}

Secondo la meccanica quantistica, in realtà, gli spin saltano su due livelli energetici: \(\left\langle - \right|\), di energia \(+ \hslash/2\) e \(\left| + \right\rangle\), di energia \(- \hslash/2\). Dal punto di vista macroscopico, ogni volumetto elementare presenta un vettore di magnetizzazione \(\vec{M}\), dato dalla somma dei singoli momenti magnetici associati ai protoni, normalizzato il volume elementare:

\[\vec{M} = \dfrac{1}{V}\sum_{i = 1}^{N_{A}}{\vec{\mu}}_{i}\]
...
\[\dfrac{d{\vec{\mu}}_{i}}{dt} = \gamma{\vec{\mu}}_{i} \times {\vec{B}}_{0}\]

Sommando i vari contributi di ogni spin del volumetto e dividendo per il volume elementare si ha:

\[\dfrac{1}{V}\sum_{i = 1}^{N_{A}}\dfrac{d{\vec{\mu}}_{i}}{dt} = \dfrac{1}{V}\sum_{i = 1}^{N_{A}}{\gamma{\vec{\mu}}_{i} \times {\vec{B}}_{0}}\]

Per la linearità dell'operatore somma e derivata è possibile scrivere:

\[\sum_{i = 1}^{N_{A}}{\dfrac{d}{dt}\left( \dfrac{1}{V}{\vec{\mu}}_{i} \right)} = \sum_{i = 1}^{N_{A}}{\gamma\dfrac{1}{V}{\vec{\mu}}_{i} \times {\vec{B}}_{0}} \Leftrightarrow \dfrac{d}{dt}\left( \sum_{i = 1}^{N_{A}}{\dfrac{1}{V}{\vec{\mu}}_{i}} \right) = \gamma\left( \sum_{i = 1}^{N_{A}}{\dfrac{1}{V}{\vec{\mu}}_{i}} \right) \times {\vec{B}}_{0}\]
...
Dove, in coordinate cartesiane e con un campo magnetico esterno diretto lungo \({\hat{i}}_{z}\):

\[{\vec{M}}_{\parallel} = M_{z}{\hat{i}}_{z}\]

\[{\vec{M}}_{\perp} = M_{x}{\hat{i}}_{x} + = M_{y}{\hat{i}}_{y}\]

L'equazione differenziale può essere scritta scomponendo il vettore di magnetizzazione:

\[\dfrac{d}{dt}\left( {\vec{M}}_{\parallel} + {\vec{M}}_{\perp} \right) = \gamma\left( {\vec{M}}_{\parallel} + {\vec{M}}_{\perp} \right) \times B_{0}{\hat{i}}_{Z}\]

Per linearità:

\[\dfrac{d{\vec{M}}_{\parallel}}{dt} + \dfrac{d{\vec{M}}_{\perp}}{dt} = \gamma B_{0}{\vec{M}}_{\parallel} \times {\hat{i}}_{Z} + \gamma B_{0}{\vec{M}}_{\perp} \times {\hat{i}}_{Z}\]

Per definizione di componente parallela del vettore di magnetizzazione, il termine \({\vec{M}}_{\parallel} \times {\hat{i}}_{Z}\) si annulla:

\[{\vec{M}}_{\parallel} \times {\hat{i}}_{Z} = M_{z}{\hat{i}}_{Z} \times {\hat{i}}_{Z} = 0\]

Per cui:

\[\dfrac{d{\vec{M}}_{\parallel}}{dt} + \dfrac{d{\vec{M}}_{\perp}}{dt} = \gamma B_{0}{\vec{M}}_{\perp} \times {\hat{i}}_{Z}\]

Scrivendo le due equazioni per le proiezioni si ha:

\[\left\{ \begin{matrix}
\dfrac{dM_{\parallel}}{dt} = 0 \\
\dfrac{d{\vec{M}}_{\perp}}{dt} = \gamma B_{0}{\vec{M}}_{\perp} \times {\hat{i}}_{Z}
\end{matrix} \right.\ \]
...
è errata poiché non tiene conto di tali fenomeni; infatti, momenti magnetici dei protoni tendono ad allinearsi col campo esterno, tuttavia, le iterazioni con l'ambiente circostante non consentono al vettore magnetizzazione di essere diretto esattamente lungo l'asse del campo principale, \({\hat{i}}_{z}\).
...
La componente lungo \({\hat{i}}_{y}\) è nulla poiché l'andamento delle linee di campo magnetico di uno spin hanno simmetria cilindrica.
...
La componente lungo \({\hat{i}}_{z}\) è legata all'energia del sistema, infatti, in meccanica classica, si prova che l'energia di un momento magnetico \(\vec{\mu}\) immerso in un campo magnetico \({\vec{B}}_{0}\) è data da:

\[U = - \vec{\mu} \cdot {\vec{B}}_{0}\]

Con un campo con solo componente verticale ovvero \({\vec{B}}_{0} = B_{0}{\hat{i}}_{z}\), si ha:

\[U = - \mu_{z}B_{0}\]

Ne discende che la componente verticale, lungo \({\hat{i}}_{z}\), è coinvolta negli scambi energetici tra dipoli elementari.

L'interpretazione della componente lungo \({\hat{i}}_{x}\) del campo prodotto da uno spin \(B_{x}\), è più complessa in quanto bisogna tener conto che i dipoli non sono fermi nello spazio ma precedono intorno all'asse individuato dal campo esterno, uno ente a influenzare questa componente.
...
Il parametro \(\mathrm{\Delta}N\) è fondamentale poiché la magnetizzazione macroscopica del volumetto elementare \(\vec{M}\) è proporzionale alla differenza di popolazione di spin nello stato \(\left| + \right\rangle\) rispetto a quelle nello stato \(\left| - \right\rangle\):

\[\left| \vec{M} \right| \propto \mathrm{\Delta}N = N^{+} - N^{-}\]
...
\[\dfrac{d\mathrm{\Delta}N}{dt} = 2N^{-}W_{- \rightarrow +} - 2N^{+}W_{+ \rightarrow -}\]
...
\[N + \mathrm{\Delta}N = 2N^{+} \Leftrightarrow N^{+} = \dfrac{1}{2}(N + \mathrm{\Delta}N)\]
...
\[\dfrac{d\mathrm{\Delta}N}{dt} = 2\dfrac{1}{2}(N - \mathrm{\Delta}N)W_{- \rightarrow +} - 2\dfrac{1}{2}(N + \mathrm{\Delta}N)W_{+ \rightarrow -} = (N - \mathrm{\Delta}N)W_{- \rightarrow +} - (N + \mathrm{\Delta}N)W_{+ \rightarrow -}\]

Raccogliendo \(N\) e \(\mathrm{\Delta}N\) al secondo membro si ha:

\[\dfrac{d\mathrm{\Delta}N}{dt} = N\left( W_{- \rightarrow +} - W_{+ \rightarrow -} \right) - \mathrm{\Delta}N\left( W_{- \rightarrow +} + W_{+ \rightarrow -} \right)\]
...
\[\mathrm{\Delta}N_{0} = N_{0}^{+} - N_{0}^{-}\]

\(\mathrm{\Delta}N_{0}\) è la differenza di spin nello stato \(\left| + \right\rangle\) rispetto a quelli nello stato \(\left| - \right\rangle\) all'equilibrio termodinamico; ciò produce la magnetizzazione macroscopica \(\vec{M}\).
...
\[\mathrm{\Delta}N_{0} = N\dfrac{W_{- \rightarrow +} - W_{+ \rightarrow -}}{W_{- \rightarrow +} + W_{+ \rightarrow -}}\]
...
\[\dfrac{d}{dt}\left( \mathrm{\Delta}N - \mathrm{\Delta}N_{0} \right) = N\left( W_{- \rightarrow +} - W_{+ \rightarrow -} \right) - \mathrm{\Delta}N\left( W_{- \rightarrow +} + W_{+ \rightarrow -} \right)\]

Al secondo membro si raccoglie il termine \(\left( W_{- \rightarrow +} + W_{+ \rightarrow -} \right)\), ottenendo:

\[\dfrac{d}{dt}\left( \mathrm{\Delta}N - \mathrm{\Delta}N_{0} \right) = \left( N\dfrac{W_{- \rightarrow +} - W_{+ \rightarrow -}}{W_{- \rightarrow +} + W_{+ \rightarrow -}} - \mathrm{\Delta}N \right)\left( W_{- \rightarrow +} + W_{+ \rightarrow -} \right)\]
...
\[\dfrac{d}{dt}\left( \mathrm{\Delta}N - \mathrm{\Delta}N_{0} \right) = \left( \mathrm{\Delta}N_{0} - \mathrm{\Delta}N \right)\left( W_{- \rightarrow +} + W_{+ \rightarrow -} \right)\]
...
\[\dfrac{d}{dt}\left( \mathrm{\Delta}N - \mathrm{\Delta}N_{0} \right) = \dfrac{1}{T_{1}}\left( \mathrm{\Delta}N_{0} - \mathrm{\Delta}N \right)\]

Al fine di avere la stessa quantità, si raccoglie un segno meno al secondo membro:

\[\dfrac{d}{dt}\left( \mathrm{\Delta}N - \mathrm{\Delta}N_{0} \right) = - \dfrac{1}{T_{1}}\left( \mathrm{\Delta}N - \mathrm{\Delta}N_{0} \right)\]
...
Il tempo \(T_{1}\) interessa gli scambi tra i vari spin contenuti nel materiale ed è legato alla componente longitudinale, lungo \({\hat{i}}_{z}\). L'equazione di Bloch \(dM_{z}/dt = 0\) viene corretta introducendo l'andamento temporale, dettato dalla costante di tempo \(T_{1}\):
...
\[M_{z}\left(t\right) = M_{z}(0)\exp\left( - \dfrac{t}{T_{1}} \right) + M_{0}\left\lbrack 1 - \exp\left( - \dfrac{t}{T_{1}} \right) \right\rbrack\]
...
Si considera un singolo spin, il campo prodotto da questo spin è indicato con \({\vec{B}}_{loc}\). Gli altri spin genereranno dei campi diversi. Tutte le componenti verticali possiedono intensità diverse, non predicibili in quanto non è possibile ricostruire il moto di ogni singolo spin del volumetto considerato.

Per studiare il campo locale visto da uno spin si utilizza il valor quadratico medio:

\[B_{RMS} = \sqrt{\left\langle \left| {\vec{B}}_{loc} \right| \right\rangle^{2}}\]

Dunque, vi è una componente magnetica del campo valor quadratico medio lungo \({\hat{i}}_{z}\). Questa componente si sovrappone al campo magnetico principale \(B_{0}{\hat{i}}_{z}\), applicato dall'esterno; ciò determina una variazione della frequenza con cui i singoli spin precedono intorno all'asse \({\hat{i}}_{z}\).
...
\[B_{RMS}\left(t\right) = \sqrt{\left\langle \left| {\vec{B}}_{loc}\left(t\right) \right| \right\rangle^{2}}\]

Sia \(t_{1}\) un primo istante di osservazione; il campo locale di uno spin vale \({\vec{B}}_{loc}\left( t_{1} \right)\). In un secondo istante temporale \(t_{2}\), successivo al primo, il campo locale sarà diverso, a causa del modi termici, per cui:
...
Per ogni spin, la frequenza di precessione intorno all'asse \({\hat{i}}_{z}\) è diversa dagli altri ed è data da:

\[\mathrm{\Delta}\omega\left(t\right) = \gamma{\vec{B}}_{loc}\left(t\right)\]

Dove \(\mathrm{\Delta}\omega = \omega_{0} - \omega_{loc}\left(t\right)\). La differenza tra le frequenze di precessione è, dunque, una funzione del tempo. Dati due istanti temporale, risulta:

\[\mathrm{\Delta}\omega\left( t_{1} \right) = \gamma{\vec{B}}_{loc}\left( t_{1} \right) \neq \mathrm{\Delta}\omega\left( t_{2} \right) = \gamma{\vec{B}}_{loc}\left( t_{2} \right)\]
...
\[\tau = - \dfrac{\pi}{\gamma\mathrm{\Delta}B_{loc}}\]
...
Il tempo di defasamento \(\tau\) è indicato con \(T_{2}\) ed è detto tempo di rilassamento traversarsele. \(T_{2}\) rappresenta la costante di tempo con cui la componente trasversale del vettore di magnetizzazione, \({\vec{M}}_{\perp}\), tende a zero:
...
\[\dfrac{d\vec{M}}{dt} = \gamma\vec{M} \times {\vec{B}}_{0} + \dfrac{1}{T_{1}}\left( M_{0} - M_{z}\  \right){\hat{i}}_{z} - \dfrac{1}{T_{2}}{\vec{M}}_{\perp}\]

La precedente equazione è detta di Bloch. Il segno meno nel termine \({\vec{M}}_{\perp}/T_{2}\) è dovuto al tendere a zero della componente traversale del vettore di magnetizzazione con un tempo \(T_{2}\).
...
\[\dfrac{d\vec{M}}{dt} = \gamma\vec{M} \times {\vec{B}}_{0} + R_{1}\left( M_{0} - M_{z}\  \right){\hat{i}}_{z} - R_{2}{\vec{M}}_{\perp}\]
...
\begin{longtable}[]{@{}
  >{\centering\arraybackslash}p{0.33\linewidth}
  >{\centering\arraybackslash}p{0.33\linewidth}
  >{\centering\arraybackslash}p{0.33\linewidth}@{}}
\caption{Tabella 6.1: Tempi di rilassamento di vari tessuti molli con campo principale di \(1.5\ \mathrm{T}\) e temperatura di \(37\,^{\circ}\mathrm{C}\)}\tabularnewline
\toprule
...
Si suppone di forzare il sistema di spin mediante un campo magnetico principale \(B_{0}\) diretto lungo \({\hat{i}}_{z}\). Si scrive il prodotto vettorale tra il vettore di magnetizzazione e il campo esterno applicato:

\[\vec{M} \times {\vec{B}}_{0} = \left| \begin{matrix}
{\hat{i}}_{x} & {\hat{i}}_{y} & {\hat{i}}_{z} \\
M_{x} & M_{y} & M_{z} \\
0 & 0 & B_{0}
\end{matrix} \right| = B_{0}\left( M_{y}{\hat{i}}_{x} - M_{x}{\hat{i}}_{y} \right)\]

Si scrive l'equazione di Bloch esplicitando le componenti dei vettori coinvolti:

\[\dfrac{d}{dt}\left( M_{x}{\hat{i}}_{x} + M_{y}{\hat{i}}_{y} + M_{z}{\hat{i}}_{z} \right) = \gamma B_{0}\left( M_{y}{\hat{i}}_{x} - M_{x}{\hat{i}}_{y} \right) + \dfrac{1}{T_{1}}\left( M_{0} - M_{z}\  \right){\hat{i}}_{z} - \dfrac{1}{T_{2}}\left( M_{x}{\hat{i}}_{x} + M_{y}{\hat{i}}_{y} \right)\]
...
\[M_{z}\left(t\right) = M_{0}\left\lbrack 1 - \exp\left( - \dfrac{t}{T_{1}} \right) \right\rbrack + M_{z}(0)\exp\left( - \dfrac{t}{T_{1}} \right)\]
...
\[{\vec{M}}_{\perp} = \vec{k}\exp\left( \lambda\textbf{I}t \right)\]

Sostituendo tale equazione nell'equazione differenziale si ha:

\[\dfrac{d}{dt}\left\lbrack \vec{k}\exp\left( \lambda\textbf{I}t \right) \right\rbrack = \begin{pmatrix}
 - \dfrac{1}{T_{2}} & \omega_{0} \\
 - \omega_{0} & - \dfrac{1}{T_{2}}
\end{pmatrix}\vec{k}\exp\left( \lambda\textbf{I}t \right) \Leftrightarrow \lambda\textbf{I}\vec{k}\exp\left( \lambda\textbf{I}t \right) = \begin{pmatrix}
 - \dfrac{1}{T_{2}} & \omega_{0} \\
 - \omega_{0} & - \dfrac{1}{T_{2}}
\end{pmatrix}\vec{k}\exp\left( \lambda\textbf{I}t \right)\]

Moltiplicando per la matrice inversa a \(\vec{k}\exp\left( \lambda\textbf{I}t \right)\) si ottiene:

\[\lambda\textbf{I} = \begin{pmatrix}
 - \dfrac{1}{T_{2}} & \omega_{0} \\
 - \omega_{0} & - \dfrac{1}{T_{2}}
...
La soluzione lungo \({\hat{i}}_{x}\) è:

\[M_{x}\left(t\right) = \left\lbrack C_{1}\exp\left( - j\omega_{0}t \right) + C_{2}\exp\left( j\omega_{0}t \right) \right\rbrack\exp\left( - \dfrac{t}{T_{2}} \right)\]

Dove:

\[C_{1}\exp\left( - j\omega_{0}t \right) = \dfrac{1}{2}\left\lbrack M_{x}\left( t_{0} \right)\exp\left( \dfrac{t_{0}}{T_{2}} \right) + jM_{y}\left( t_{0} \right)\exp\left( \dfrac{t_{0}}{T_{2}} \right) \right\rbrack\exp\left( j\omega_{0}t_{0} \right)\exp\left( - j\omega_{0}t \right) =\]

\[= \left\lbrack \dfrac{1}{2}M_{x}\left( t_{0} \right) + j\dfrac{1}{2}M_{y}\left( t_{0} \right) \right\rbrack\exp\left( \dfrac{t_{0}}{T_{2}} \right)\exp\left\lbrack - j\omega_{0}\left( t - t_{0} \right) \right\rbrack = \left\lbrack \dfrac{1}{2}M_{x}\left( t_{0} \right) - \dfrac{1}{2j}M_{y}\left( t_{0} \right) \right\rbrack\exp\left( \dfrac{t_{0}}{T_{2}} \right)\exp\left\lbrack - j\omega_{0}\left( t - t_{0} \right) \right\rbrack\]

\[C_{2}\exp\left( j\omega_{0}t \right) = \dfrac{1}{2}\left\lbrack M_{x}\left( t_{0} \right)\exp\left( \dfrac{t_{0}}{T_{2}} \right) - jM_{y}\left( t_{0} \right)\exp\left( \dfrac{t_{0}}{T_{2}} \right) \right\rbrack\exp\left( - j\omega_{0}t_{0} \right)\exp\left( j\omega_{0}t \right) = \left\lbrack \dfrac{1}{2}M_{x}\left( t_{0} \right) - j\dfrac{1}{2}M_{y}\left( t_{0} \right) \right\rbrack\exp\left( - j\omega_{0}t_{0} \right)\exp\left\lbrack j\omega_{0}\left( t - t_{0} \right) \right\rbrack = \left\lbrack \dfrac{1}{2}M_{x}\left( t_{0} \right) + \dfrac{1}{2j}M_{y}\left( t_{0} \right) \right\rbrack\exp\left( - j\omega_{0}t_{0} \right)\exp\left\lbrack j\omega_{0}\left( t - t_{0} \right) \right\rbrack\]

Quindi, la soluzione lungo \({\hat{i}}_{x}\) si scrive come:

\[M_{x}\left(t\right) = \left\{ \left\lbrack \dfrac{1}{2}M_{x}\left( t_{0} \right) - \dfrac{1}{2j}M_{y}\left( t_{0} \right) \right\rbrack\exp\left( \dfrac{t_{0}}{T_{2}} \right)\exp\left\lbrack - j\omega_{0}\left( t - t_{0} \right) \right\rbrack + \left\lbrack \dfrac{1}{2}M_{x}\left( t_{0} \right) - j\dfrac{1}{2}M_{y}\left( t_{0} \right) \right\rbrack\exp\left( - j\omega_{0}t_{0} \right)\exp\left\lbrack j\omega_{0}\left( t - t_{0} \right) \right\rbrack \right\}\exp\left( - \dfrac{t}{T_{2}} \right)\]

Raccogliendo opportunamente si ha:

\[M_{x}\left(t\right) = \left\{ M_{x}\left( t_{0} \right)\dfrac{\exp\left\lbrack j\omega_{0}\left( t - t_{0} \right) \right\rbrack + \exp\left\lbrack - j\omega_{0}\left( t - t_{0} \right) \right\rbrack}{2} + M_{y}\left( t_{0} \right)\dfrac{\exp\left\lbrack j\omega_{0}\left( t - t_{0} \right) \right\rbrack - \exp\left\lbrack - j\omega_{0}\left( t - t_{0} \right) \right\rbrack}{2j} \right\}\exp\left( - \dfrac{t - t_{0}}{T_{2}} \right)\]
...
Analogamente, per la componente lungo \({\hat{i}}_{y}\) si ha:

\[M_{y}\left(t\right) = \left\lbrack - jC_{1}\exp\left( - j\omega_{0}t \right) + jC_{2}\exp\left( j\omega_{0}t \right) \right\rbrack\exp\left( - \dfrac{t}{T_{2}} \right)\]

Dove:

\[- jC_{1}\exp\left( - j\omega_{0}t \right) = - j\dfrac{1}{2}\left\lbrack M_{x}\left( t_{0} \right)\exp\left( \dfrac{t_{0}}{T_{2}} \right) + jM_{y}\left( t_{0} \right)\exp\left( \dfrac{t_{0}}{T_{2}} \right) \right\rbrack\exp\left( j\omega_{0}t_{0} \right)\exp\left( - j\omega_{0}t \right) = \left\lbrack - j\dfrac{1}{2}M_{x}\left( t_{0} \right) + \dfrac{1}{2}M_{y}\left( t_{0} \right) \right\rbrack\exp\left( \dfrac{t_{0}}{T_{2}} \right)\exp\left\lbrack - j\omega_{0}\left( t - t_{0} \right) \right\rbrack = \left\lbrack \dfrac{1}{2j}M_{x}\left( t_{0} \right) + \dfrac{1}{2}M_{y}\left( t_{0} \right) \right\rbrack\exp\left( \dfrac{t_{0}}{T_{2}} \right)\exp\left\lbrack - j\omega_{0}\left( t - t_{0} \right) \right\rbrack\]

\[jC_{2}\exp\left( j\omega_{0}t \right) = j\dfrac{1}{2}\left\lbrack M_{x}\left( t_{0} \right)\exp\left( \dfrac{t_{0}}{T_{2}} \right) - jM_{y}\left( t_{0} \right)\exp\left( \dfrac{t_{0}}{T_{2}} \right) \right\rbrack\exp\left( - j\omega_{0}t_{0} \right)\exp\left( j\omega_{0}t \right) = = \left\lbrack j\dfrac{1}{2}M_{x}\left( t_{0} \right) + \dfrac{1}{2}M_{y}\left( t_{0} \right) \right\rbrack\exp\left( \dfrac{t_{0}}{T_{2}} \right)\exp\left\lbrack j\omega_{0}\left( t - t_{0} \right) \right\rbrack =\]

\[= \left\lbrack - \dfrac{1}{2j}M_{x}\left( t_{0} \right) + \dfrac{1}{2}M_{y}\left( t_{0} \right) \right\rbrack\exp\left( \dfrac{t_{0}}{T_{2}} \right)\exp\left\lbrack j\omega_{0}\left( t - t_{0} \right) \right\rbrack\]

Quindi, la soluzione lungo \({\hat{i}}_{y}\) si scrive come:

\[M_{y}\left(t\right) = \left\{ \left\lbrack \dfrac{1}{2j}M_{x}\left( t_{0} \right) + \dfrac{1}{2}M_{y}\left( t_{0} \right) \right\rbrack\exp\left( \dfrac{t_{0}}{T_{2}} \right)\exp\left\lbrack - j\omega_{0}\left( t - t_{0} \right) \right\rbrack + \left\lbrack - \dfrac{1}{2j}M_{x}\left( t_{0} \right) + \dfrac{1}{2}M_{y}\left( t_{0} \right) \right\rbrack\exp\left( \dfrac{t_{0}}{T_{2}} \right)\exp\left\lbrack j\omega_{0}\left( t - t_{0} \right) \right\rbrack \right\}\exp\left( - \dfrac{t}{T_{2}} \right)\]

Raccogliendo opportunamente si ha:

\[M_{y}\left(t\right) = \left\{ - M_{x}\left( t_{0} \right)\dfrac{\exp\left\lbrack j\omega_{0}\left( t - t_{0} \right) \right\rbrack - \exp\left\lbrack - j\omega_{0}\left( t - t_{0} \right) \right\rbrack}{2j} + M_{y}\left( t_{0} \right)\dfrac{\exp\left\lbrack j\omega_{0}\left( t - t_{0} \right) \right\rbrack + \exp\left\lbrack - j\omega_{0}\left( t - t_{0} \right) \right\rbrack}{2} \right\}\exp\left( - \dfrac{t - t_{0}}{T_{2}} \right)\]
...
\[\left\{ \begin{matrix}
M_{x}\left(t\right) = \left\lbrack M_{x}(0)\cos\left( \omega_{0}t \right) + M_{y}(0)\sin\left( \omega_{0}t \right) \right\rbrack\exp\left( - \dfrac{t}{T_{2}} \right) \\
M_{y}\left(t\right) = \left\lbrack - M_{x}(0)\sin\left( \omega_{0}t \right) + M_{y}(0)\cos\left( \omega_{0}t \right) \right\rbrack\exp\left( - \dfrac{t - t_{0}}{T_{2}} \right)
\end{matrix} \right.\ \]
...
\[M_{+}\left(t\right) = M_{x}\left(t\right) + jM_{y}\left(t\right) = \left\lbrack M_{x}(0)\cos\left( \omega_{0}t \right) + M_{y}(0)\sin\left( \omega_{0}t \right) - jM_{x}(0)\sin\left( \omega_{0}t \right) + jM_{y}(0)\cos\left( \omega_{0}t \right) \right\rbrack\exp\left( - \dfrac{t}{T_{2}} \right)\]

Raccogliendo si ottiene:

\[M_{+}\left(t\right) = \left\{ M_{x}(0)\left\lbrack \cos\left( \omega_{0}t \right) - j\sin\left( \omega_{0}t \right) \right\rbrack + jM_{y}(0)\left\lbrack \cos\left( \omega_{0}t \right) - j\sin\left( \omega_{0}t \right) \right\rbrack \right\}\exp\left( - \dfrac{t}{T_{2}} \right) = \left\{ \left\lbrack \cos\left( \omega_{0}t \right) - j\sin\left( \omega_{0}t \right) \right\rbrack\left\lbrack M_{x}(0) + jM_{y}(0) \right\rbrack \right\}\exp\left( - \dfrac{t}{T_{2}} \right)\]
...
\[M_{+}\left(t\right) = M_{+}(0)\exp\left( - j\omega_{0}t \right)\exp\left( - \dfrac{t}{T_{2}} \right) = M_{+}(0)\exp\left\lbrack - \left( j\omega_{0}t + \dfrac{t}{T_{2}} \right) \right\rbrack\ \]
...
\[\vec{M}(0) = M_{x}(0){\hat{i}}_{x} + M_{y}(0){\hat{i}}_{y}\]

La componente lungo \({\hat{i}}_{z}\) parte dal valore nullo e si porta al valore di regime in un tempo di \(5 \div 10\ s\), mentre quelle trasversali vanno a zero in un tempo di \(0.5 \div 1\ s\).

La curva descritta dal vettore di magnetizzazione tende a raggiungere il valore di regime \(M_{0}\) sull'asse delle \({\hat{i}}_{z}\) mediante un andamento a spirale, convergente sull'asse del campo principale. Il modo elicoidale a raggio variabile lo si ritrova dopo una perturbazione ed è il moto in cui il vettore \(\vec{M}\) torna all'equilibrio termodinamico per effetto del campo principale \(B_{0}\). I tempi di rilassamento, quindi, interessano il ritorno all'equilibrio.
...
Il campo efficace, visto da uno spin nel sistema di riferimento rotante al quale è applicato un impulso a radiofrequenza diretto lungo l'asse \({\hat{i}}_{x'}\), è:

\[{\vec{B}_{\text{eff}}} = \left( B_{0} - \dfrac{\omega}{\gamma} \right){\hat{i}}_{z} + B_{1}{\hat{i}}_{x'}\]
...
\[\left( \dfrac{d\vec{M}}{dt} \right)' = \gamma\vec{M} \times {\vec{B}_{\text{eff}}} + \dfrac{1}{T_{1}}\left( M_{0} - M_{z}\  \right){\hat{i}}_{z} - \dfrac{1}{T_{2}}{\vec{M}}_{\perp}\]

Questa equazione è stata ottenuta senza applicare nessuna ipotesi sul sistema di riferimento, quindi, è valida sia in sistemi inerziali che non inerziali. In particolare, la componente longitudinale del vettore di magnetizzazione evolve in maniera indipendente da quelle trasversali, quindi, il modo lungo l'asse \({\hat{i}}_{z}\) si conserva.

Si proietta l'equazione vettoriale di Bloch lungo gli assi, svolgendo il prodotto vettoriale:

\[\vec{M} \times {\vec{B}_{\text{eff}}} = \left| \begin{matrix}
{\hat{i}}_{x'} & {\hat{i}}_{y'} & {\hat{i}}_{z}\  \\
M_{x'} & M_{y'} & M_{z} \\
B_{1} & 0 & B_{0} - \dfrac{\omega}{\gamma}
\end{matrix} \right| = M_{y'}\left( B_{0} - \dfrac{\omega}{\gamma} \right){\hat{i}}_{x'} + M_{z}B_{1}{\hat{i}}_{y'} - M_{y'}B_{1}{\hat{i}}_{z} - M_{x'}\left( B_{0} - \dfrac{\omega}{\gamma} \right){\hat{i}}_{y'}\]

Raccogliendo i termini, si ottiene:

\[\vec{M} \times {\vec{B}_{\text{eff}}} = M_{y'}\left( B_{0} - \dfrac{\omega}{\gamma} \right){\hat{i}}_{x'} + M_{z}B_{1}{\hat{i}}_{y'} - M_{y'}B_{1}{\hat{i}}_{z} - M_{x'}\left( B_{0} - \dfrac{\omega}{\gamma} \right){\hat{i}}_{y'} = M_{y'}\left( B_{0} - \dfrac{\omega}{\gamma} \right){\hat{i}}_{x'} - \left\lbrack M_{x'}\left( B_{0} - \dfrac{\omega}{\gamma} \right) - M_{z}B_{1} \right\rbrack{\hat{i}}_{y'} - M_{y'}B_{1}{\hat{i}}_{z}\]

L'equazione vettoriale di Bloch si scrive:

\[\left( \dfrac{d\vec{M}}{dt} \right)' = \gamma\vec{M} \times {\vec{B}_{\text{eff}}} + \dfrac{1}{T_{1}}\left( M_{0} - M_{z}\  \right){\hat{i}}_{z} - \dfrac{1}{T_{2}}{\vec{M}}_{\perp} = \gamma\left\{ M_{y'}\left( B_{0} - \dfrac{\omega}{\gamma} \right){\hat{i}}_{x'} - \left\lbrack M_{x'}\left( B_{0} - \dfrac{\omega}{\gamma} \right) - M_{z}B_{1} \right\rbrack{\hat{i}}_{y'} - M_{y'}B_{1}{\hat{i}}_{z} \right\} + \dfrac{1}{T_{1}}\left( M_{0} - M_{z}\  \right){\hat{i}}_{z} - \dfrac{1}{T_{2}}\left( M_{x'}{\hat{i}}_{x'} + M_{y'}{\hat{i}}_{y'} \right)\]
...
Le equazioni individuate suggeriscono un moto di precessione intorno l'asse \({\hat{i}}_{x'}\).
...
\begin{figure}[ht]
  ...
  \begin{minipage}[b]{0.45\textwidth}
    \centering
    \includegraphics[width=3.17495in,height=3.19514in,alt={P3542C1T5\#yIS1}]{media/6_IntroMRI/image78.pdf}
    \label{fig:img1}
  \end{minipage}
  \hfill
  \begin{minipage}[b]{0.45\textwidth}
    \centering
    \includegraphics[width=2.90572in,height=3.19514in,alt={P3543C2T5\#yIS1}]{media/6_IntroMRI/image79.pdf}
    \label{fig:img2}
  \end{minipage}
...
Il segnale registrato è proporzionale alla componente trasversa del campo, dunque, le antenne ricevono una sinusoide, a frequenza \(\omega_{0}\) smorzata con un decadimento di tipo esponenziale e costante di tempo \(T_{2}\). Adoperando la notazione complessa, il segnale registrato dall'antenna è proporzionale al fasore \(M_{+}\left(t\right)\):

\[M_{+}\left(t\right) = M_{+}(0)\exp\left( - \dfrac{t}{T_{2}} \right)\exp\left( - j\omega_{0}t \right)\]
