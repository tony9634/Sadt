...
\[
\mathbf{R}_{z}\left( \omega_{0}t \right) = \left| \begin{matrix}
\cos\left( \omega_{0}t \right) & - \sin\left( \omega_{0}t \right) \\
\sin\left( \omega_{0}t \right) & \cos\left( \omega_{0}t \right)
...
M_{x} & M_{y} & M_{z} \\
0 & 0 & B_{0}
\end{matrix} \right| = B_{0}\left( M_{y}{\hat{\imath}}_{x} - M_{x}{\hat{\imath}}_{y} \right)\]
...
\[M_{+}\left(t\right) = M_{x}\left(t\right) + jM_{y}\left(t\right) = \left\lbrack M_{x}\left(0\right)\cos\left( \omega_{0}t \right) + M_{y}\left(0\right)\sin\left( \omega_{0}t \right) - jM_{x}\left(0\right)\sin\left( \omega_{0}t \right) + jM_{y}\left(0\right)\cos\left( \omega_{0}t \right) \right\rbrack\exp\left( - \dfrac{t}{T_{2}} \right)\]

Raccogliendo si ottiene:

\[M_{+}\left(t\right) = \left\{ M_{x}\left(0\right)\left\lbrack \cos\left( \omega_{0}t \right) - j\sin\left( \omega_{0}t \right) \right\rbrack + jM_{y}\left(0\right)\left\lbrack \cos\left( \omega_{0}t \right) - j\sin\left( \omega_{0}t \right) \right\rbrack \right\}\exp\left( - \dfrac{t}{T_{2}} \right) = \left\{ \left\lbrack \cos\left( \omega_{0}t \right) - j\sin\left( \omega_{0}t \right) \right\rbrack\left\lbrack M_{x}\left(0\right) + jM_{y}\left(0\right) \right\rbrack \right\}\exp\left( - \dfrac{t}{T_{2}} \right)\]
...
Il campo efficace, visto da uno spin nel sistema di riferimento rotante al quale è applicato un impulso a radiofrequenza diretto lungo l'asse \({\hat{\imath}}_{x'}\), è:
...
\end{matrix} \right| = M_{y'}\left( B_{0} - \dfrac{\omega}{\gamma} \right){\hat{\imath}}_{x'} + M_{z}B_{1}{\hat{\imath}}_{y'} - M_{y'}B_{1}{\hat{\imath}}_{z} - M_{x'}\left( B_{0} - \dfrac{\omega}{\gamma} \right){\hat{\imath}}_{y'}\]

Raccogliendo i termini, si ottiene:

\[\vec{M} \times {\vec{B}_{\text{eff}}} = M_{y'}\left( B_{0} - \dfrac{\omega}{\gamma} \right){\hat{\imath}}_{x'} + M_{z}B_{1}{\hat{\imath}}_{y'} - M_{y'}B_{1}{\hat{\imath}}_{z} - M_{x'}\left( B_{0} - \dfrac{\omega}{\gamma} \right){\hat{\imath}}_{y'} = M_{y'}\left( B_{0} - \dfrac{\omega}{\gamma} \right){\hat{\imath}}_{x'} - \left\lbrack M_{x'}\left( B_{0} - \dfrac{\omega}{\gamma} \right) - M_{z}B_{1} \right\rbrack{\hat{\imath}}_{y'} - M_{y'}B_{1}{\hat{\imath}}_{z}\]

L'equazione vettoriale di Bloch si scrive:

\[\left( \dfrac{d\vec{M}}{dt} \right)' = \gamma\vec{M} \times {\vec{B}_{\text{eff}}} + \dfrac{1}{T_{1}}\left( M_{0} - M_{z}\  \right){\hat{\imath}}_{z} - \dfrac{1}{T_{2}}{\vec{M}}_{\perp} = \gamma\left\{ M_{y'}\left( B_{0} - \dfrac{\omega}{\gamma} \right){\hat{\imath}}_{x'} - \left\lbrack M_{x'}\left( B_{0} - \dfrac{\omega}{\gamma} \right) - M_{z}B_{1} \right\rbrack{\hat{\imath}}_{y'} - M_{y'}B_{1}{\hat{\imath}}_{z} \right\} + \dfrac{1}{T_{1}}\left( M_{0} - M_{z}\  \right){\hat{\imath}}_{z} - \dfrac{1}{T_{2}}\left( M_{x'}{\hat{\imath}}_{x'} + M_{y'}{\hat{\imath}}_{y'} \right)\]
...
È possibile, quindi, trascurare i termini contenenti \(\Delta \omega\):
...
Le equazioni individuate suggeriscono un moto di precessione intorno l'asse \({\hat{\imath}}_{x'}\).
...
Dove \(M_{+}\left(0\right)\) è proporzionale al vettore di magnetizzazione all'equilibrio termodinamico \(M_{0}\), che a sua volta dipende dalla densità protonica \(\rho\), indicante il numero di protoni nel volumetto \(V\).
